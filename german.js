 const german = [
 ["ich", "I"],
 ["sie", "you"],
 ["das", "the"],
 ["ist", "is"],
 ["du", "you"],
 ["nicht", "Not"],
 ["die", "the"],
 ["und", "and"],
 ["es", "it"],
 ["der", "of the"],
 ["was", "What"],
 ["wir", "we"],
 ["er", "he"],
 ["zu", "to"],
 ["ein", "one"],
 ["in", "in"],
 ["mit", "With"],
 ["mir", "me"],
 ["den", "the"],
 ["wie", "how"],
 ["ja", "Yes"],
 ["auf", "on"],
 ["mich", "me"],
 ["so", "so"],
 ["eine", "a"],
 ["aber", "but"],
 ["hier", "here"],
 ["sind", "are"],
 ["für", "For"],
 ["von", "from"],
 ["haben", "to have"],
 ["hat", "Has"],
 ["dich", "you"],
 ["war", "was"],
 ["dass", "that"],
 ["wenn", "if"],
 ["an", "on"],
 ["da", "there"],
 ["nein", "No"],
 ["bin", "am"],
 ["noch", "yet"],
 ["dir", "to you"],
 ["nur", "just"],
 ["habe", "have"],
 ["ihr", "their"],
 ["sich", "themselves"],
 ["einen", "one"],
 ["uns", "us"],
 ["hast", "have"],
 ["dem", "the"],
 ["ihn", "him"],
 ["aus", "out"],
 ["kann", "can"],
 ["gut", "Good"],
 ["auch", "also"],
 ["schon", "beautiful"],
 ["jetzt", "now"],
 ["im", "in the"],
 ["sein", "be"],
 ["mal", "times"],
 ["dann", "then"],
 ["meine", "my"],
 ["als", "when"],
 ["um", "around"],
 ["mein", "my"],
 ["doch", "but"],
 ["bist", "are"],
 ["wird", "becomes"],
 ["keine", "no"],
 ["nach", "to"],
 ["alles", "everything"],
 ["man", "you"],
 ["lch", "l"],
 ["oder", "or"],
 ["nichts", "Nothing"],
 ["wo", "Where"],
 ["werden", "will"],
 ["weiß", "White"],
 ["will", "want"],
 ["geht", "going"],
 ["mehr", "more"],
 ["warum", "Why"],
 ["hab", "hab"],
 ["ihnen", "them"],
 ["bitte", "Sorry"],
 ["etwas", "something"],
 ["bei", "at"],
 ["muss", "got to"],
 ["los", "Come on"],
 ["immer", "always"],
 ["oh", "Oh"],
 ["vor", "in front"],
 ["mann", "man"],
 ["zum", "to the"],
 ["wieder", "again"],
 ["sehr", "very"],
 ["sehen", "see"],
 ["sagen", "say"],
 ["also", "so"],
 ["gehen", "go"],
 ["wer", "who"],
 ["alle", "all"],
 ["denn", "because"],
 ["machen", "do"],
 ["ihm", "him"],
 ["können", "can"],
 ["diese", "this"],
 ["komm", "come over"],
 ["danke", "Thank you"],
 ["euch", "to you"],
 ["einem", "one"],
 ["tun", "to do"],
 ["einer", "one"],
 ["gibt", "gives"],
 ["nie", "never"],
 ["über", "about"],
 ["des", "of"],
 ["soll", "should"],
 ["kein", "no"],
 ["vielleicht", "perhaps"],
 ["weg", "path"],
 ["deine", "yours"],
 ["wissen", "knowledge"],
 ["am", "at the"],
 ["werde", "will"],
 ["leben", "Life"],
 ["kommen", "come"],
 ["kommt", "coming"],
 ["müssen", "have to"],
 ["viel", "a lot of"],
 ["wirklich", "for real"],
 ["frau", "Mrs"],
 ["hatte", "would have"],
 ["heute", "today"],
 ["ok", "OK"],
 ["willst", "want"],
 ["dein", "your"],
 ["würde", "would"],
 ["tut", "does"],
 ["ihre", "your"],
 ["ganz", "all"],
 ["zeit", "time"],
 ["bis", "to"],
 ["wollen", "want"],
 ["einfach", "easy"],
 ["macht", "power"],
 ["gott", "God"],
 ["zurück", "back"],
 ["nun", "now"],
 ["weil", "because"],
 ["damit", "in order to"],
 ["dieser", "this"],
 ["kannst", "can"],
 ["wurde", "has been"],
 ["gesagt", "told"],
 ["wäre", "would"],
 ["seine", "his"],
 ["wollte", "wanted to"],
 ["na", "n / A"],
 ["zwei", "two"],
 ["hallo", "Hello"],
 ["meinen", "mean"],
 ["sicher", "for sure"],
 ["weißt", "know"],
 ["sir", "sir"],
 ["morgen", "morning"],
 ["ab", "from"],
 ["leid", "suffering"],
 ["lassen", "to let"],
 ["hey", "hey"],
 ["waren", "were"],
 ["zur", "to"],
 ["lass", "let"],
 ["geld", "money"],
 ["liebe", "love"],
 ["tag", "Day"],
 ["leute", "People"],
 ["vater", "father"],
 ["genau", "I agree"],
 ["sagte", "said"],
 ["hätte", "would have"],
 ["mach", "mach"],
 ["raus", "Out"],
 ["durch", "by"],
 ["lhr", "LHR"],
 ["könnte", "could"],
 ["schön", "beautiful"],
 ["wohl", "well"],
 ["gesehen", "seen"],
 ["keinen", "no"],
 ["klar", "clear"],
 ["glaube", "faith"],
 ["her", "here"],
 ["okay", "OK"],
 ["mutter", "mother"],
 ["sag", "say"],
 ["dieses", "this"],
 ["nacht", "night"],
 ["besser", "better"],
 ["ohne", "without"],
 ["unsere", "our"],
 ["jemand", "someone"],
 ["sei", "be"],
 ["reden", "talk"],
 ["gerade", "straight"],
 ["ob", "if"],
 ["gehört", "heard"],
 ["geh", "go"],
 ["möchte", "would like to"],
 ["dort", "there"],
 ["sagt", "says"],
 ["anderen", "other"],
 ["gemacht", "made"],
 ["hör", "hear"],
 ["sollte", "should"],
 ["selbst", "self"],
 ["diesen", "this"],
 ["gute", "quality"],
 ["dachte", "thought"],
 ["paar", "pair"],
 ["weiter", "continue"],
 ["vom", "from"],
 ["ins", "in the"],
 ["herr", "Mr"],
 ["wirst", "will"],
 ["geben", "give"],
 ["ordnung", "order"],
 ["passiert", "happens"],
 ["meiner", "mine"],
 ["lange", "Long"],
 ["gar", "at all"],
 ["mädchen", "girl"],
 ["meinem", "my"],
 ["hören", "Listen"],
 ["sieht", "looks"],
 ["daß", "that"],
 ["guten", "good"],
 ["hin", "down"],
 ["gleich", "equal"],
 ["ach", "Oh"],
 ["diesem", "this"],
 ["freund", "friend"],
 ["seit", "since"],
 ["welt", "world"],
 ["musst", "must"],
 ["hause", "home"],
 ["natürlich", "Naturally"],
 ["abend", "Eve"],
 ["angst", "fear"],
 ["drei", "three"],
 ["recht", "Law"],
 ["richtig", "right"],
 ["viele", "lots"],
 ["deinen", "your"],
 ["finden", "Find"],
 ["wieso", "how so"],
 ["bleiben", "stay"],
 ["tot", "dead"],
 ["unter", "under"],
 ["junge", "Boy"],
 ["haus", "House"],
 ["rein", "purely"],
 ["essen", "eat"],
 ["davon", "from that"],
 ["nehmen", "to take"],
 ["sollen", "should"],
 ["helfen", "help"],
 ["schnell", "fast"],
 ["machst", "doing"],
 ["eines", "one"],
 ["andere", "other"],
 ["unser", "our"],
 ["warte", "wait"],
 ["wegen", "because of"],
 ["menschen", "People"],
 ["stimmt", "Right"],
 ["dafür", "for this"],
 ["darf", "may"],
 ["kinder", "children"],
 ["genug", "enough"],
 ["sonst", "otherwise"],
 ["ganze", "whole"],
 ["scheiße", "crap"],
 ["he", "hey"],
 ["halt", "stop"],
 ["sollten", "should"],
 ["zusammen", "together"],
 ["gegen", "versus"],
 ["jahre", "years"],
 ["erst", "first"],
 ["denke", "think"],
 ["steht", "stands"],
 ["habt", "have"],
 ["verdammt", "damned"],
 ["moment", "moment"],
 ["ihren", "their"],
 ["glauben", "believe"],
 ["bringen", "bring"],
 ["gib", "give"],
 ["seid", "are"],
 ["niemand", "nobody"],
 ["warten", "waiting"],
 ["brauchen", "need"],
 ["lhre", "Their"],
 ["fragen", "ask"],
 ["arbeit", "job"],
 ["wann", "when"],
 ["heißt", "called"],
 ["sprechen", "speak"],
 ["siehst", "see"],
 ["jeder", "everyone"],
 ["kam", "came"],
 ["sofort", "immediately"],
 ["getan", "done"],
 ["fertig", "finished"],
 ["kennen", "know"],
 ["einmal", "once"],
 ["sohn", "son"],
 ["halten", "hold"],
 ["dabei", "there"],
 ["hatten", "had"],
 ["männer", "Men"],
 ["kind", "child"],
 ["jahren", "years"],
 ["beim", "at the"],
 ["seinen", "his"],
 ["mag", "like"],
 ["komme", "get"],
 ["allein", "alone"],
 ["bevor", "before"],
 ["bruder", "brothers"],
 ["musik", "music"],
 ["wahr", "true"],
 ["konnte", "could"],
 ["lieber", "Dear"],
 ["uhr", "clock"],
 ["kopf", "head"],
 ["sache", "thing"],
 ["gern", "gladly"],
 ["denken", "think"],
 ["brauche", "need"],
 ["ende", "the end"],
 ["später", "later"],
 ["gehe", "go"],
 ["vergessen", "to forget"],
 ["runter", "down"],
 ["fahren", "drive"],
 ["sieh", "look"],
 ["warst", "were"],
 ["stadt", "city"],
 ["namen", "names"],
 ["problem", "problem"],
 ["sehe", "see"],
 ["augen", "eyes"],
 ["gab", "gave"],
 ["dank", "thanks"],
 ["sage", "legend"],
 ["würden", "would"],
 ["eins", "one"],
 ["daran", "it"],
 ["dazu", "to"],
 ["egal", "no matter"],
 ["frage", "question"],
 ["weit", "far"],
 ["familie", "family"],
 ["sterben", "to die"],
 ["könnten", "could"],
 ["jungs", "guys"],
 ["jeden", "each"],
 ["minuten", "minutes"],
 ["verstehe", "understand"],
 ["deiner", "yours"],
 ["hilfe", "help"],
 ["beide", "both"],
 ["bald", "soon"],
 ["all", "Alles"],
 ["deinem", "your"],
 ["nimm", "take"],
 ["kleine", "small"],
 ["baby", "infant"],
 ["glück", "happiness"],
 ["freunde", "friends"],
 ["wusste", "knew"],
 ["verrückt", "insane"],
 ["ruhig", "calm"],
 ["land", "country"],
 ["darüber", "about that"],
 ["gefunden", "found"],
 ["tür", "door"],
 ["mache", "make"],
 ["seiner", "his"],
 ["wasser", "water"],
 ["auto", "automobile"],
 ["sah", "saw"],
 ["dies", "this"],
 ["eigentlich", "actually"],
 ["eure", "your"],
 ["ruhe", "quiet"],
 ["stehen", "stand"],
 ["hand", "hand"],
 ["ihrer", "of their"],
 ["frauen", "women"],
 ["meinst", "mean"],
 ["töten", "kill"],
 ["fall", "case"],
 ["ging", "went"],
 ["polizei", "police"],
 ["vorbei", "past"],
 ["krieg", "war"],
 ["je", "ever"],
 ["darauf", "thereon"],
 ["vielen", "a lot of"],
 ["letzte", "latest"],
 ["lang", "long"],
 ["bisschen", "little"],
 ["dran", "tuned"],
 ["hoch", "high"],
 ["kenne", "know"],
 ["fast", "nearly"],
 ["ganzen", "all"],
 ["schau", "show"],
 ["sogar", "even"],
 ["spielen", "play"],
 ["wurden", "were"],
 ["hinter", "Behind"],
 ["teufel", "devil"],
 ["dinge", "things"],
 ["keiner", "none"],
 ["mama", "mummy"],
 ["spät", "late"],
 ["bekommen", "to get"],
 ["oben", "above"],
 ["name", "surname"],
 ["hört", "hears"],
 ["hätten", "had"],
 ["bereit", "ready"],
 ["drin", "in it"],
 ["neue", "new"],
 ["kommst", "coming"],
 ["woher", "from where"],
 ["ihrem", "their"],
 ["wagen", "dare"],
 ["arbeiten", "work"],
 ["verstehen", "understand"],
 ["jahr", "year"],
 ["tod", "death"],
 ["etwa", "approximately"],
 ["echt", "real"],
 ["seinem", "his"],
 ["geschichte", "story"],
 ["bringt", "brings"],
 ["braucht", "need"],
 ["treffen", "to meet"],
 ["toll", "Great"],
 ["gerne", "with pleasure"],
 ["draußen", "outside"],
 ["fünf", "five"],
 ["lhnen", "wages sent"],
 ["zimmer", "room"],
 ["job", "job"],
 ["nett", "kind"],
 ["weiss", "white"],
 ["the", "the"],
 ["spiel", "game"],
 ["verstanden", "understood"],
 ["miss", "ill"],
 ["große", "size"],
 ["anders", "different"],
 ["sorgen", "to care"],
 ["welche", "which"],
 ["einzige", "single"],
 ["art", "kind"],
 ["liegt", "lies"],
 ["verloren", "lost"],
 ["ahnung", "idea"],
 ["vier", "four"],
 ["alter", "Age"],
 ["gekommen", "came"],
 ["jemanden", "someone"],
 ["gehst", "going"],
 ["woche", "week"],
 ["kerl", "Guy"],
 ["gestern", "yesterday"],
 ["wenig", "little"],
 ["bestimmt", "certainly"],
 ["kurz", "short"],
 ["überhaupt", "at all"],
 ["finde", "find"],
 ["darum", "therefore"],
 ["tage", "days"],
 ["erste", "first"],
 ["schwester", "sister"],
 ["schwer", "heavy"],
 ["suchen", "search"],
 ["bleibt", "remains"],
 ["tochter", "daughter"],
 ["typ", "Type"],
 ["guter", "goods"],
 ["gewesen", "been"],
 ["grund", "reason"],
 ["zeigen", "demonstrate"],
 ["film", "Movie"],
 ["schlecht", "bad"],
 ["schatz", "sweetheart"],
 ["deshalb", "therefore"],
 ["holen", "to fetch"],
 ["hoffe", "hope"],
 ["tu", "tu"],
 ["sagst", "say"],
 ["bett", "bed"],
 ["muß", "got to"],
 ["seite", "page"],
 ["gefällt", "like"],
 ["eben", "just"],
 ["lasst", "leaves"],
 ["gefallen", "like"],
 ["stunden", "hours"],
 ["herz", "heart"],
 ["wohin", "where"],
 ["trinken", "drink"],
 ["letzten", "last"],
 ["mensch", "human"],
 ["ersten", "first"],
 ["unten", "below"],
 ["gesicht", "face"],
 ["versuchen", "to attempt"],
 ["wort", "word"],
 ["denen", "to those"],
 ["verlassen", "leave"],
 ["endlich", "Finally"],
 ["denkst", "think"],
 ["solltest", "should"],
 ["schlafen", "sleep"],
 ["ziemlich", "quite"],
 ["arsch", "ass"],
 ["manchmal", "sometimes"],
 ["fest", "firmly"],
 ["wiedersehen", "meet again"],
 ["beste", "best"],
 ["kleinen", "small"],
 ["kriegen", "Obtain"],
 ["läuft", "running"],
 ["wahrheit", "truth"],
 ["zwischen", "between"],
 ["niemals", "No way"],
 ["bring", "bring"],
 ["unserer", "our"],
 ["nehme", "take"],
 ["allen", "all"],
 ["während", "while"],
 ["glaubst", "think"],
 ["ding", "thing"],
 ["idee", "idea"],
 ["wollten", "wanted to"],
 ["lässt", "leaves"],
 ["musste", "had to"],
 ["sachen", "stuff"],
 ["ah", "Ah"],
 ["bleib", "permanent"],
 ["kleiner", "smaller"],
 ["euer", "your"],
 ["nummer", "number"],
 ["sondern", "rather"],
 ["frei", "free"],
 ["oft", "often"],
 ["entschuldigung", "Excuse me"],
 ["wen", "whom"],
 ["ernst", "serious"],
 ["alte", "old"],
 ["schule", "school"],
 ["ort", "place"],
 ["erzählt", "tells"],
 ["anderes", "other"],
 ["new", "new"],
 ["alt", "old"],
 ["chance", "chance"],
 ["einige", "some"],
 ["bloß", "just"],
 ["entschuldigen", "to apologize"],
 ["setzen", "put"],
 ["bedeutet", "means"],
 ["erzählen", "tell"],
 ["feuer", "Fire"],
 ["pass", "passport"],
 ["würdest", "would"],
 ["schuld", "fault"],
 ["dad", "dad"],
 ["getötet", "killed"],
 ["jungen", "boys"],
 ["drauf", "it"],
 ["platz", "place"],
 ["wären", "would"],
 ["tust", "do"],
 ["papa", "father"],
 ["spaß", "fun"],
 ["versucht", "tries"],
 ["wichtig", "important"],
 ["besten", "best"],
 ["sorge", "worry"],
 ["kennst", "know"],
 ["lst", "lst"],
 ["hund", "dog"],
 ["blut", "blood"],
 ["hierher", "here"],
 ["reicht", "enough"],
 ["alten", "old"],
 ["wär", "would"],
 ["glücklich", "happy"],
 ["brauchst", "need"],
 ["hände", "hands"],
 ["unseren", "our"],
 ["hättest", "had"],
 ["kaum", "barely"],
 ["rede", "speech"],
 ["zuerst", "first"],
 ["stellen", "put"],
 ["beiden", "both"],
 ["falls", "if"],
 ["früh", "early"],
 ["wartet", "waiting"],
 ["neuen", "new"],
 ["sechs", "six"],
 ["jede", "each"],
 ["freundin", "girlfriend"],
 ["lieben", "love"],
 ["teil", "part"],
 ["stunde", "hour"],
 ["langsam", "slowly"],
 ["scheint", "seems"],
 ["früher", "earlier"],
 ["großen", "huge"],
 ["waffe", "weapon"],
 ["telefon", "phone"],
 ["himmel", "sky"],
 ["lernen", "learn"],
 ["äh", "um"],
 ["zehn", "ten"],
 ["gedacht", "thought"],
 ["willkommen", "welcome"],
 ["frank", "frank"],
 ["luft", "air"],
 ["licht", "light"],
 ["hol", "hol"],
 ["seht", "look"],
 ["menge", "quantity"],
 ["voll", "full"],
 ["überall", "all over"],
 ["tat", "did"],
 ["erde", "earth"],
 ["gebe", "give"],
 ["ruf", "call"],
 ["möglich", "possible"],
 ["spricht", "speaks"],
 ["könig", "king"],
 ["eltern", "parents"],
 ["außer", "except"],
 ["nächsten", "next"],
 ["ziehen", "pull"],
 ["wahrscheinlich", "probably"],
 ["wollt", "want"],
 ["dürfen", "allowed to"],
 ["nennen", "call"],
 ["falsch", "not correct"],
 ["ehrlich", "honest"],
 ["irgendwas", "anything"],
 ["nächste", "next"],
 ["wochen", "weeks"],
 ["meines", "mine"],
 ["herren", "Men's"],
 ["straße", "road"],
 ["hm", "Hm"],
 ["heiraten", "marry"],
 ["stück", "piece"],
 ["sam", "sam"],
 ["doktor", "doctor"],
 ["neues", "new"],
 ["gegeben", "given"],
 ["kaffee", "coffee"],
 ["möchten", "want"],
 ["verstehst", "understand"],
 ["krank", "ill"],
 ["danach", "after that"],
 ["geworden", "become"],
 ["arzt", "doctor"],
 ["schaffen", "create"],
 ["stelle", "Job"],
 ["groß", "big"],
 ["wem", "whom"],
 ["hölle", "hell"],
 ["völlig", "completely"],
 ["weh", "sore"],
 ["fehler", "error"],
 ["drüben", "over there"],
 ["harry", "harry"],
 ["büro", "office"],
 ["captain", "captain"],
 ["rufen", "call"],
 ["irgendwie", "somehow"],
 ["eigenen", "own"],
 ["probleme", "problems"],
 ["allem", "all"],
 ["wert", "value"],
 ["tue", "do"],
 ["buch", "book"],
 ["lebt", "lives"],
 ["scheiß", "shit"],
 ["kaufen", "to buy"],
 ["erinnern", "recall"],
 ["direkt", "directly"],
 ["laufen", "to run"],
 ["george", "George"],
 ["trotzdem", "Nevertheless"],
 ["schreiben", "write"],
 ["still", "quiet"],
 ["kleines", "small"],
 ["körper", "body"],
 ["retten", "save"],
 ["fand", "found"],
 ["unserem", "our"],
 ["solche", "such"],
 ["denkt", "thinks"],
 ["könnt", "can"],
 ["ln", "ln"],
 ["alleine", "alone"],
 ["sitzen", "to sit"],
 ["höre", "hear"],
 ["millionen", "millions"],
 ["tagen", "meet"],
 ["york", "York"],
 ["suche", "search"],
 ["vergiss", "forget"],
 ["gutes", "good"],
 ["leider", "Unfortunately"],
 ["sagten", "said"],
 ["typen", "types"],
 ["schlüssel", "key"],
 ["tragen", "carry"],
 ["mund", "mouth"],
 ["worden", "been"],
 ["hält", "keeps"],
 ["zwar", "though"],
 ["bereits", "already"],
 ["leicht", "light"],
 ["spielt", "plays"],
 ["schiff", "ship"],
 ["hörst", "hear"],
 ["onkel", "uncle"],
 ["glaub", "think"],
 ["verlieren", "to lose"],
 ["großer", "greater"],
 ["klingt", "sounds"],
 ["irgendwo", "somewhere"],
 ["zukunft", "future"],
 ["kennt", "knows"],
 ["plan", "plan"],
 ["schöne", "lovely"],
 ["verschwinden", "disappear"],
 ["interessiert", "Interested"],
 ["waffen", "weapons"],
 ["kämpfen", "fight"],
 ["party", "party"],
 ["bringe", "bring"],
 ["hinten", "behind"],
 ["setz", "translated"],
 ["kumpel", "Dude"],
 ["wolltest", "wanted"],
 ["boden", "ground"],
 ["zeig", "show"],
 ["böse", "evil"],
 ["halte", "think"],
 ["gegangen", "went"],
 ["richtige", "right one"],
 ["liebling", "Darling"],
 ["acht", "eight"],
 ["froh", "glad"],
 ["machte", "made"],
 ["jedes", "each"],
 ["hart", "hard"],
 ["vorsichtig", "careful"],
 ["joe", "joe"],
 ["versuch", "attempt"],
 ["getroffen", "met"],
 ["mögen", "to like"],
 ["zeug", "stuff"],
 ["damals", "back then"],
 ["stimme", "voice"],
 ["gefühl", "feeling"],
 ["leuten", "people"],
 ["lesen", "read"],
 ["mist", "damn"],
 ["zug", "train"],
 ["hattest", "had"],
 ["verdammte", "damned"],
 ["geschäft", "business"],
 ["arschloch", "asshole"],
 ["rest", "rest"],
 ["mom", "mom"],
 ["de", "de"],
 ["aufhören", "stop"],
 ["könntest", "could"],
 ["klasse", "great"],
 ["lasse", "leave"],
 ["boss", "boss"],
 ["erklären", "to explain"],
 ["fliegen", "to fly"],
 ["laden", "load"],
 ["stolz", "proud"],
 ["stark", "strongly"],
 ["hasse", "hate"],
 ["wohnung", "flat"],
 ["gebracht", "brought"],
 ["fort", "on"],
 ["genauso", "just like that"],
 ["mai", "May"],
 ["entschuldige", "Excuse me"],
 ["besonders", "especially"],
 ["seien", "are"],
 ["tages", "day"],
 ["weniger", "fewer"],
 ["zieh", "pulling"],
 ["ärger", "trouble"],
 ["fühle", "feel"],
 ["schauen", "watch"],
 ["schneller", "more quickly"],
 ["denk", "thinking"],
 ["geschafft", "made"],
 ["verzeihung", "forgiveness"],
 ["daddy", "daddy"],
 ["charlie", "charlie"],
 ["tja", "tja"],
 ["schicken", "send"],
 ["erwartet", "expected"],
 ["nachricht", "message"],
 ["findet", "finds"],
 ["funktioniert", "is working"],
 ["werd", "expectant"],
 ["stand", "was standing"],
 ["traum", "dream"],
 ["sollst", "shalt"],
 ["fallen", "fall"],
 ["wovon", "About what"],
 ["gefängnis", "jail"],
 ["aufs", "onto"],
 ["hotel", "hotel"],
 ["krankenhaus", "hospital"],
 ["umbringen", "kill"],
 ["fällt", "falls"],
 ["redest", "talking about"],
 ["weisst", "know"],
 ["gottes", "god"],
 ["total", "total"],
 ["fenster", "window"],
 ["lebens", "life"],
 ["glaubt", "believes"],
 ["vorstellen", "to introduce"],
 ["and", "and"],
 ["sinn", "sense"],
 ["ändern", "to change"],
 ["versuche", "tries"],
 ["heraus", "out"],
 ["heißen", "be called"],
 ["tanzen", "to dance"],
 ["monate", "months"],
 ["lachen", "laugh"],
 ["kampf", "struggle"],
 ["nachdem", "after this"],
 ["arm", "poor"],
 ["wenigstens", "at least"],
 ["geschehen", "done"],
 ["hörte", "heard"],
 ["kamen", "came"],
 ["passt", "fit"],
 ["liebt", "loves"],
 ["folgen", "consequences"],
 ["eher", "rather"],
 ["gefragt", "asked"],
 ["dumm", "stupid"],
 ["deswegen", "therefore"],
 ["solange", "as long as"],
 ["lustig", "funny"],
 ["starb", "died"],
 ["unmöglich", "impossible"],
 ["bild", "image"],
 ["boot", "boat"],
 ["genommen", "took"],
 ["dame", "lady"],
 ["führen", "to lead"],
 ["unglaublich", "incredible"],
 ["vertrauen", "trust"],
 ["verletzt", "injured"],
 ["passieren", "happen"],
 ["cool", "cool"],
 ["meisten", "most"],
 ["tom", "tom"],
 ["vorher", "previously"],
 ["plötzlich", "suddenly"],
 ["angerufen", "called"],
 ["bewegung", "Move"],
 ["kümmern", "To take care of"],
 ["rüber", "over"],
 ["komisch", "funny"],
 ["paris", "Paris"],
 ["super", "Super"],
 ["sieben", "seven"],
 ["erinnere", "remember"],
 ["lhrer", "lhrer"],
 ["ne", "ne"],
 ["rufe", "call"],
 ["bier", "beer"],
 ["augenblick", "moment"],
 ["herum", "around"],
 ["kalt", "cold"],
 ["wunderbar", "wonderful"],
 ["großartig", "Great"],
 ["wahl", "choice"],
 ["auge", "eye"],
 ["fahr", "driving"],
 ["konnten", "could"],
 ["länger", "longer"],
 ["tisch", "table"],
 ["rum", "rum"],
 ["antwort", "answer"],
 ["jedem", "each"],
 ["david", "David"],
 ["arme", "poor"],
 ["umgebracht", "killed"],
 ["schlimm", "bad"],
 ["verheiratet", "married"],
 ["steh", "stand"],
 ["anrufen", "call"],
 ["worte", "worte"],
 ["möchtest", "want"],
 ["vorsicht", "Attention"],
 ["hilf", "help"],
 ["bezahlt", "paid"],
 ["voller", "full of"],
 ["links", "Left"],
 ["sekunden", "seconds"],
 ["werdet", "will"],
 ["liegen", "lie"],
 ["fühlen", "feel"],
 ["nochmal", "again"],
 ["armee", "army"],
 ["bitten", "ask"],
 ["gestorben", "died"],
 ["finger", "finger"],
 ["meinung", "opinion"],
 ["nimmt", "takes"],
 ["zahlen", "numbers"],
 ["sicherheit", "safety"],
 ["verkaufen", "to sell"],
 ["gesprochen", "spoken"],
 ["wozu", "what for"],
 ["müde", "tired"],
 ["seele", "soul"],
 ["welcher", "which one"],
 ["schießen", "shoot"],
 ["michael", "Michael"],
 ["arbeite", "working"],
 ["sitzt", "seated"],
 ["to", "to"],
 ["sonne", "Sun"],
 ["damen", "Ladies"],
 ["laß", "let"],
 ["bezahlen", "pay"],
 ["selber", "himself"],
 ["reise", "travel"],
 ["team", "team"],
 ["sobald", "as soon as"],
 ["meister", "master"],
 ["majestät", "majesty"],
 ["anfang", "Beginning"],
 ["fangen", "to catch"],
 ["außerdem", "Furthermore"],
 ["geschrieben", "written"],
 ["sex", "sex"],
 ["hole", "get"],
 ["kriegt", "gets"],
 ["yeah", "yeah"],
 ["magst", "like"],
 ["laut", "loud"],
 ["klappe", "flap"],
 ["amerika", "America"],
 ["nahm", "took"],
 ["niemanden", "anyone"],
 ["gefährlich", "dangerous"],
 ["mörder", "killer"],
 ["gewinnen", "win"],
 ["gedanken", "thoughts"],
 ["obwohl", "even though"],
 ["monat", "month"],
 ["mike", "mike"],
 ["arbeitet", "is working"],
 ["herrn", "Mr"],
 ["behalten", "to keep"],
 ["legen", "lay"],
 ["bob", "bob"],
 ["person", "person"],
 ["herzen", "heart"],
 ["brief", "letter"],
 ["präsident", "president"],
 ["rechts", "right"],
 ["anfangen", "to begin"],
 ["schluss", "Enough"],
 ["hübsch", "pretty"],
 ["don", "Don"],
 ["gewonnen", "won"],
 ["wisst", "know"],
 ["versprochen", "promised"],
 ["chef", "boss"],
 ["findest", "find"],
 ["perfekt", "Perfect"],
 ["schlagen", "beat"],
 ["tatsächlich", "indeed"],
 ["pferd", "horse"],
 ["dagegen", "on the other hand"],
 ["erfahren", "Experienced"],
 ["monsieur", "monsieur"],
 ["mord", "murder"],
 ["paul", "paul"],
 ["nähe", "neighborhood"],
 ["nötig", "necessary"],
 ["aller", "all"],
 ["offen", "open"],
 ["wach", "awake"],
 ["singen", "to sing"],
 ["beispiel", "example"],
 ["ais", "ais"],
 ["sowieso", "anyway"],
 ["jedenfalls", "in any case"],
 ["schwein", "pig"],
 ["bank", "Bank"],
 ["ziel", "target"],
 ["klein", "small"],
 ["verdient", "earned"],
 ["band", "tape"],
 ["geist", "ghost"],
 ["ma", "ma"],
 ["max", "Max"],
 ["neu", "New"],
 ["schreit", "screams"],
 ["anwalt", "lawyer"],
 ["monaten", "months"],
 ["show", "show"],
 ["heim", "home"],
 ["führt", "leads"],
 ["lauf", "run"],
 ["ehre", "honor"],
 ["unterwegs", "on road"],
 ["gelernt", "learned"],
 ["minute", "minute"],
 ["reich", "rich"],
 ["wunder", "wonder"],
 ["opfer", "Victim"],
 ["erzähl", "tell"],
 ["bekannt", "known"],
 ["hau", "hau"],
 ["haut", "skin"],
 ["weder", "neither"],
 ["rücken", "move"],
 ["bekommt", "gets"],
 ["peter", "Peter"],
 ["jemals", "ever"],
 ["partner", "partner"],
 ["zieht", "attracts"],
 ["nase", "nose"],
 ["gefahr", "danger"],
 ["regeln", "regulate"],
 ["absolut", "absolutely"],
 ["tief", "deep"],
 ["flugzeug", "plane"],
 ["befehl", "command"],
 ["geschickt", "cleverly"],
 ["wette", "bet"],
 ["nennt", "is called"],
 ["jung", "young"],
 ["sucht", "investigated"],
 ["gehabt", "had"],
 ["geboren", "born"],
 ["rolle", "role"],
 ["schönen", "beautiful"],
 ["holt", "get"],
 ["erinnerst", "remember"],
 ["heiße", "name is"],
 ["kraft", "force"],
 ["lhrem", "lhrem"],
 ["lacht", "laughs"],
 ["bar", "bar"],
 ["richtung", "direction"],
 ["tasche", "bag"],
 ["schlag", "blow"],
 ["professor", "professor"],
 ["steckt", "plugged"],
 ["ständig", "constantly"],
 ["erwischt", "Caught"],
 ["stell", "Vice"],
 ["schrecklich", "dreadful"],
 ["verschwinde", "disappear"],
 ["weise", "wise"],
 ["schlimmer", "worse"],
 ["ließ", "was"],
 ["frieden", "peace"],
 ["verliebt", "in love"],
 ["bleibe", "stay"],
 ["ehe", "marriage"],
 ["kriege", "wars"],
 ["näher", "closer"],
 ["gelesen", "read"],
 ["dahin", "then"],
 ["wofür", "for what"],
 ["dauert", "lasts"],
 ["neben", "Next"],
 ["gold", "gold"],
 ["raum", "room"],
 ["benutzt", "used"],
 ["welches", "which"],
 ["regierung", "government"],
 ["fährt", "moves"],
 ["volk", "people"],
 ["fernsehen", "watch TV"],
 ["zeiten", "times"],
 ["toten", "kill"],
 ["achtung", "Danger"],
 ["karte", "map"],
 ["hunger", "hunger"],
 ["diesmal", "this time"],
 ["preis", "price"],
 ["benutzen", "to use"],
 ["scheisse", "Shit"],
 ["hilft", "help"],
 ["geschenk", "gift"],
 ["hoffentlich", "hopefully"],
 ["manche", "some"],
 ["ansehen", "look at"],
 ["kriegst", "get"],
 ["lage", "location"],
 ["wünschte", "wish"],
 ["fehlt", "is missing"],
 ["willen", "sake"],
 ["bescheid", "decision"],
 ["heiß", "hot"],
 ["kamera", "camera"],
 ["gleiche", "same"],
 ["ruft", "calls"],
 ["unfall", "accident"],
 ["ans", "ans"],
 ["bullen", "cops"],
 ["szene", "scene"],
 ["gericht", "dish"],
 ["gerettet", "rescued"],
 ["schlaf", "sleep"],
 ["schicksal", "fate"],
 ["gesetz", "law"],
 ["fürs", "for"],
 ["verkauft", "sold"],
 ["bewegen", "move"],
 ["leiche", "corpse"],
 ["soldaten", "soldiers"],
 ["geburtstag", "birthday"],
 ["lebe", "live"],
 ["darfst", "allowed to"],
 ["ewig", "forever"],
 ["schade", "what a shame"],
 ["witz", "joke"],
 ["müsste", "should"],
 ["erwarten", "expect"],
 ["aiies", "aiies"],
 ["ihres", "their"],
 ["fahre", "ride"],
 ["ähm", "um"],
 ["pro", "Per"],
 ["traurig", "Sad"],
 ["hältst", "hold"],
 ["doc", "doc"],
 ["jemandem", "somebody"],
 ["setzt", "puts"],
 ["gesellschaft", "society"],
 ["punkt", "Point"],
 ["freiheit", "freedom"],
 ["jimmy", "Jimmy"],
 ["darin", "in this"],
 ["brachte", "brought"],
 ["weile", "while"],
 ["jawohl", "Yes"],
 ["dasselbe", "the same thing"],
 ["maul", "mouth"],
 ["zeigt", "shows"],
 ["taxi", "taxi"],
 ["erreichen", "to reach"],
 ["redet", "talks"],
 ["foto", "photo"],
 ["gekauft", "Bought"],
 ["meer", "sea"],
 ["sheriff", "sheriff"],
 ["liebst", "love"],
 ["unbedingt", "absolutely"],
 ["aussehen", "look"],
 ["tante", "aunt"],
 ["drehen", "rotate"],
 ["stirbt", "dies"],
 ["bord", "shelf"],
 ["kirche", "church"],
 ["tee", "tea"],
 ["of", "of"],
 ["erledigt", "done"],
 ["wein", "Wine"],
 ["weihnachten", "Christmas"],
 ["irgendwann", "Sometime"],
 ["billy", "billy"],
 ["euren", "your"],
 ["brauch", "custom"],
 ["hochzeit", "wedding"],
 ["dorf", "Village"],
 ["sagtest", "told"],
 ["james", "james"],
 ["lady", "lady"],
 ["lust", "lust"],
 ["kontrolle", "control"],
 ["tolle", "amazing"],
 ["beine", "legs"],
 ["hut", "cap"],
 ["wiii", "wiii"],
 ["hoffnung", "hope"],
 ["firma", "company"],
 ["me", "me"],
 ["lügen", "lying"],
 ["sergeant", "sergeant"],
 ["fürchte", "fear"],
 ["kaputt", "broken"],
 ["bekam", "got"],
 ["is", "is"],
 ["nachrichten", "news"],
 ["ball", "ball"],
 ["haare", "hair"],
 ["tschüss", "bye"],
 ["seltsam", "strange"],
 ["vergnügen", "pleasure"],
 ["sauber", "clean"],
 ["bill", "bill"],
 ["zuhause", "At home"],
 ["anderer", "another"],
 ["zeichen", "character"],
 ["herein", "in"],
 ["liste", "list"],
 ["haltet", "think"],
 ["normal", "normal"],
 ["see", "lake"],
 ["schätze", "wealth"],
 ["bewegt", "emotional"],
 ["wussten", "knew"],
 ["blick", "view"],
 ["naja", "well"],
 ["gesucht", "searched"],
 ["gestohlen", "stolen"],
 ["vorne", "ahead"],
 ["jim", "jim"],
 ["kindern", "children"],
 ["frag", "question"],
 ["bleibst", "stay"],
 ["nachts", "at night"],
 ["neun", "nine"],
 ["nämlich", "namely"],
 ["wunderschön", "beautiful"],
 ["stecken", "stick"],
 ["großes", "great"],
 ["freut", "pleased"],
 ["weitere", "Further"],
 ["gehören", "belong"],
 ["schwöre", "swear"],
 ["nervös", "nervous"],
 ["hals", "neck"],
 ["gearbeitet", "worked"],
 ["eigene", "own"],
 ["glas", "Glass"],
 ["versteckt", "hidden"],
 ["feind", "enemy"],
 ["schönes", "beautiful"],
 ["verzeihen", "forgive"],
 ["öffnen", "to open"],
 ["meter", "meter"],
 ["drogen", "drugs"],
 ["schöner", "more beautiful"],
 ["nehmt", "take"],
 ["madame", "madame"],
 ["mary", "mary"],
 ["süß", "sweet"],
 ["entscheidung", "decision"],
 ["erschossen", "shot"],
 ["verändert", "changed"],
 ["la", "la"],
 ["wind", "wind"],
 ["ray", "ray"],
 ["wohnen", "live"],
 ["schuhe", "Shoes"],
 ["interessant", "Interesting"],
 ["übrigens", "by the way"],
 ["letzter", "last"],
 ["müsst", "need"],
 ["nächstes", "next"],
 ["schaut", "looks"],
 ["armen", "poor"],
 ["verstand", "understanding"],
 ["verraten", "betray"],
 ["besuch", "visit"],
 ["beweise", "proofs"],
 ["sekunde", "second"],
 ["polizist", "police officer"],
 ["meint", "means"],
 ["fur", "For"],
 ["insel", "island"],
 ["beweisen", "to prove"],
 ["antworten", "reply"],
 ["fotos", "photos"],
 ["passen", "fit"],
 ["werfen", "throw"],
 ["persönlich", "personally"],
 ["fragte", "asked"],
 ["namens", "called"],
 ["gegenüber", "across from"],
 ["unsinn", "nonsense"],
 ["fuß", "foot"],
 ["besuchen", "visit"],
 ["güte", "quality"],
 ["lieb", "dear"],
 ["guck", "peep"],
 ["deines", "yours"],
 ["roger", "roger"],
 ["richtigen", "right"],
 ["übrig", "left"],
 ["major", "major"],
 ["bein", "leg"],
 ["messer", "knife"],
 ["geheimnis", "secret"],
 ["kannte", "knew"],
 ["gegessen", "eaten"],
 ["ungefähr", "approximately"],
 ["ran", "ran"],
 ["maria", "Maria"],
 ["nahe", "Near"],
 ["meinte", "said"],
 ["nimmst", "take"],
 ["gruppe", "group"],
 ["bücher", "Books"],
 ["weib", "woman"],
 ["geschlafen", "slept"],
 ["letztes", "last"],
 ["zeitung", "newspaper"],
 ["zufrieden", "satisfied"],
 ["verdammten", "damned"],
 ["zweite", "second"],
 ["geliebt", "loved"],
 ["freude", "joy"],
 ["loch", "hole"],
 ["stellt", "provides"],
 ["statt", "instead of"],
 ["fleisch", "flesh"],
 ["ganzes", "whole"],
 ["verschwunden", "disappeared"],
 ["ee", "ee"],
 ["worauf", "whereupon"],
 ["sauer", "angry"],
 ["erkennen", "detect"],
 ["weißen", "white"],
 ["entfernt", "away"],
 ["nick", "nick"],
 ["mut", "courage"],
 ["mitnehmen", "take"],
 ["anruf", "Call"],
 ["lied", "song"],
 ["träume", "dreams"],
 ["for", "for"],
 ["kostet", "costs"],
 ["fisch", "fish"],
 ["amerikaner", "American"],
 ["schuldig", "guilty"],
 ["zumindest", "at least"],
 ["entweder", "either"],
 ["mark", "mark"],
 ["sprach", "spoke"],
 ["informationen", "information"],
 ["miteinander", "together"],
 ["sowas", "like this"],
 ["gibst", "give"],
 ["sahen", "saw"],
 ["schmerz", "pain"],
 ["furchtbar", "awful"],
 ["niemandem", "no one"],
 ["vaters", "father"],
 ["vorwärts", "forward"],
 ["computer", "computer"],
 ["teilen", "divide"],
 ["gefahren", "hazards"],
 ["erster", "first"],
 ["eier", "eggs"],
 ["seines", "his"],
 ["kontakt", "Contact"],
 ["situation", "situation"],
 ["bus", "bus"],
 ["verstecken", "hide"],
 ["händen", "hands"],
 ["ehren", "to honor"],
 ["entlang", "along"],
 ["küche", "kitchen"],
 ["wütend", "angry"],
 ["mami", "mommy"],
 ["fühlt", "feels"],
 ["eddie", "eddie"],
 ["überraschung", "surprise"],
 ["tony", "tony"],
 ["haar", "hair"],
 ["schwert", "sword"],
 ["schreien", "scream"],
 ["lag", "was"],
 ["fräulein", "young lady"],
 ["ha", "Ha"],
 ["hielt", "held"],
 ["richter", "judge"],
 ["gespielt", "played"],
 ["gegend", "area"],
 ["red", "red"],
 ["gebäude", "building"],
 ["geredet", "spake"],
 ["rat", "advice"],
 ["schlechte", "bad"],
 ["süße", "sweet"],
 ["hängt", "hangs"],
 ["eis", "ice"],
 ["big", "big"],
 ["zerstört", "destroyed"],
 ["koffer", "suitcase"],
 ["wohnt", "lives"],
 ["schwarze", "black"],
 ["versteht", "understood"],
 ["kümmere", "take care"],
 ["erledigen", "take care of"],
 ["lieutenant", "lieutenant"],
 ["alex", "Alex"],
 ["männern", "men"],
 ["witzig", "funny"],
 ["schließlich", "in the end"],
 ["position", "position"],
 ["drinnen", "indoors"],
 ["fbi", "fbi"],
 ["soweit", "so far"],
 ["falsche", "FALSE"],
 ["entscheiden", "decide"],
 ["erhalten", "receive"],
 ["bauen", "to build"],
 ["kleid", "dress"],
 ["dachten", "thought"],
 ["übel", "evil"],
 ["überleben", "to survive"],
 ["hinaus", "out"],
 ["beeil", "hurry"],
 ["wald", "Forest"],
 ["töte", "kill"],
 ["beschäftigt", "employed"],
 ["ring", "ring"],
 ["verbindung", "connection"],
 ["königin", "queen"],
 ["ecke", "corner"],
 ["halb", "half"],
 ["bericht", "report"],
 ["gingen", "went"],
 ["vergangenheit", "past"],
 ["spiele", "games"],
 ["angefangen", "started"],
 ["weinen", "cry"],
 ["welchen", "which"],
 ["radio", "radio"],
 ["nah", "close"],
 ["beziehung", "relationship"],
 ["hängen", "hang"],
 ["feiern", "to celebrate"],
 ["mussten", "had"],
 ["verhaftet", "arrested"],
 ["maschine", "machine"],
 ["ermordet", "murdered"],
 ["aiie", "aII"],
 ["richard", "Richard"],
 ["größte", "greatest"],
 ["betrunken", "drunk"],
 ["mußt", "must"],
 ["soldat", "soldier"],
 ["irgendetwas", "anything"],
 ["on", "on"],
 ["gefühle", "emotions"],
 ["rauf", "up"],
 ["rief", "cried"],
 ["beginnt", "starts"],
 ["witze", "joking"],
 ["engel", "Angel"],
 ["quatsch", "nonsense"],
 ["hälfte", "half"],
 ["spreche", "speak"],
 ["lehrer", "teacher"],
 ["england", "England"],
 ["charles", "charles"],
 ["dessen", "whose"],
 ["schwierig", "difficult"],
 ["fang", "fishing"],
 ["gleichen", "equal"],
 ["respekt", "respect"],
 ["adresse", "address"],
 ["brücke", "bridge"],
 ["falschen", "FALSE"],
 ["blumen", "flowers"],
 ["bobby", "Bobby"],
 ["besorgen", "to get"],
 ["sommer", "summer"],
 ["bilder", "pictures"],
 ["park", "park"],
 ["flug", "flight"],
 ["agent", "agent"],
 ["schlampe", "slut"],
 ["hunde", "dogs"],
 ["geschlagen", "beaten"],
 ["beginnen", "begin"],
 ["danken", "to thank"],
 ["schritt", "step"],
 ["zerstören", "to destroy"],
 ["au", "au"],
 ["martin", "martin"],
 ["bombe", "bomb"],
 ["san", "san"],
 ["schwarzen", "black"],
 ["ted", "ted"],
 ["gelassen", "calmly"],
 ["aufgabe", "task"],
 ["jerry", "jerry"],
 ["allerdings", "Indeed"],
 ["unseres", "our"],
 ["washington", "Washington"],
 ["prinzessin", "princess"],
 ["colonel", "colonel"],
 ["kümmert", "takes care"],
 ["treten", "to step"],
 ["unterschied", "difference"],
 ["gesund", "healthy"],
 ["stimmen", "be right"],
 ["knast", "slammer"],
 ["fahrt", "journey"],
 ["heisst", "called"],
 ["steig", "sidewalk"],
 ["gehirn", "brain"],
 ["besorgt", "concerned"],
 ["fluss", "River"],
 ["tiere", "animals"],
 ["bekomme", "get"],
 ["brüder", "brothers"],
 ["indem", "by doing"],
 ["trägt", "wearing"],
 ["gewartet", "waited"],
 ["bisher", "so far"],
 ["tor", "gate"],
 ["glückwunsch", "congratulation"],
 ["tommy", "tommy"],
 ["gebt", "give"],
 ["schafft", "creates"],
 ["planeten", "planet"],
 ["wünsche", "Wishes"],
 ["blöd", "stupid"],
 ["rom", "Rome"],
 ["hintern", "butt"],
 ["falle", "cases"],
 ["singt", "sings"],
 ["dach", "top, roof"],
 ["tötet", "killing"],
 ["helfe", "help"],
 ["pistole", "pistol"],
 ["freunden", "friends"],
 ["karten", "cards"],
 ["leiden", "To suffer"],
 ["wünschen", "to wish"],
 ["gewehr", "gun"],
 ["fühlst", "feel"],
 ["kapitän", "captain"],
 ["neuer", "new"],
 ["reichen", "pass"],
 ["system", "system"],
 ["sprich", "speak"],
 ["schläft", "is sleeping"],
 ["worüber", "about what"],
 ["katze", "cat"],
 ["vogel", "bird"],
 ["rennen", "run"],
 ["erinnert", "remind"],
 ["rechte", "right"],
 ["bringst", "bring"],
 ["tier", "animal"],
 ["steigen", "climb"],
 ["flasche", "bottle"],
 ["traf", "hit"],
 ["küssen", "kiss"],
 ["deren", "their"],
 ["leisten", "Afford"],
 ["halbe", "half"],
 ["eurer", "yours"],
 ["verspreche", "promise"],
 ["vermisst", "missing"],
 ["genannt", "called"],
 ["fragt", "asks"],
 ["verbrechen", "crime"],
 ["aussieht", "looks"],
 ["melden", "Report"],
 ["stehe", "am"],
 ["verantwortlich", "responsible"],
 ["pferde", "horses"],
 ["erfolg", "success"],
 ["lee", "lee"],
 ["ohren", "ears"],
 ["bad", "bath"],
 ["keinem", "any"],
 ["weiße", "white"],
 ["besonderes", "special"],
 ["theater", "theatre"],
 ["natur", "nature"],
 ["lächeln", "smile"],
 ["club", "club"],
 ["auftrag", "assignment"],
 ["wichser", "wanker"],
 ["aufhalten", "stop"],
 ["schmerzen", "pain"],
 ["by", "by"],
 ["davor", "before"],
 ["wüsste", "know"],
 ["abendessen", "dinner"],
 ["sarah", "sarah"],
 ["verdienen", "to earn"],
 ["trifft", "meets"],
 ["deutschen", "German"],
 ["überrascht", "surprised"],
 ["baum", "tree"],
 ["kosten", "costs"],
 ["leer", "empty"],
 ["morgens", "In the morning"],
 ["gefangen", "captured"],
 ["van", "van"],
 ["wand", "wall"],
 ["city", "City"],
 ["haufen", "heap"],
 ["schließen", "shut down"],
 ["möglichkeit", "possibility"],
 ["schützen", "protect"],
 ["besteht", "consists"],
 ["rauchen", "smoke"],
 ["schwierigkeiten", "trouble"],
 ["stein", "stone"],
 ["mitten", "in the middle"],
 ["hinein", "in"],
 ["hose", "trousers"],
 ["autos", "cars"],
 ["zuvor", "before"],
 ["prima", "great"],
 ["schwanz", "tail"],
 ["lächerlich", "ridiculous"],
 ["schickt", "sends"],
 ["wieviel", "how much"],
 ["beschützen", "to protect"],
 ["liebes", "dear"],
 ["befehle", "commands"],
 ["echte", "real"],
 ["vermutlich", "presumably"],
 ["englisch", "English"],
 ["bedeuten", "mean"],
 ["junger", "younger"],
 ["lüge", "lie"],
 ["gemeinsam", "together"],
 ["pause", "Break"],
 ["versteh", "understand"],
 ["aufpassen", "watch out"],
 ["einverstanden", "I Agree"],
 ["brechen", "break"],
 ["grad", "Degree"],
 ["prinz", "prince"],
 ["polizisten", "police officers"],
 ["lord", "lord"],
 ["druck", "print"],
 ["heilige", "saint"],
 ["daher", "therefore"],
 ["schwarz", "black"],
 ["solchen", "such a"],
 ["übernehmen", "take"],
 ["nachmittag", "afternoon"],
 ["regen", "rain"],
 ["schätzchen", "baby"],
 ["my", "my"],
 ["zweiten", "second"],
 ["rock", "skirt"],
 ["fassen", "take"],
 ["tote", "dead"],
 ["fiel", "fell"],
 ["madam", "madam"],
 ["mitgebracht", "brought"],
 ["mannes", "man"],
 ["mochte", "would like to"],
 ["wart", "serviceable"],
 ["lief", "ran"],
 ["dorthin", "there"],
 ["unterhalten", "to chat"],
 ["schuss", "shot"],
 ["angriff", "attack"],
 ["freue", "look"],
 ["gebaut", "built"],
 ["gebrochen", "Broken"],
 ["fanden", "found"],
 ["aiso", "aiso"],
 ["beruhige", "calm down"],
 ["offensichtlich", "obviously"],
 ["versprechen", "promise"],
 ["monster", "monster"],
 ["fängt", "catches"],
 ["kapiert", "Got it"],
 ["ehemann", "husband"],
 ["geschlossen", "closed"],
 ["frühstück", "breakfast"],
 ["gebeten", "asked"],
 ["stören", "to disturb"],
 ["irre", "crazy"],
 ["farbe", "colour"],
 ["chris", "chris"],
 ["taten", "doings"],
 ["zufällig", "coincidentally"],
 ["geschieht", "happens"],
 ["pfund", "lb"],
 ["mission", "mission"],
 ["thomas", "Thomas"],
 ["zeige", "show"],
 ["wochenende", "weekend"],
 ["unternehmen", "Companies"],
 ["fantastisch", "fantastic"],
 ["kate", "cottage"],
 ["geb", "give"],
 ["schlägt", "beats"],
 ["entschieden", "decided"],
 ["spur", "track"],
 ["liebte", "loved"],
 ["hieß", "was called"],
 ["vertrag", "contract"],
 ["dienst", "service"],
 ["held", "hero"],
 ["verpasst", "lost"],
 ["irgendwelche", "any"],
 ["filme", "movies"],
 ["stock", "floor"],
 ["louis", "louis"],
 ["meilen", "miles"],
 ["wahnsinn", "madness"],
 ["erschießen", "shoot dead"],
 ["nee", "nope"],
 ["einzigen", "single"],
 ["mitkommen", "come along"],
 ["braut", "bride"],
 ["reisen", "to travel"],
 ["gewissen", "conscience"],
 ["schmeckt", "tastes"],
 ["sprache", "language"],
 ["begann", "started"],
 ["eigenes", "own"],
 ["reihe", "line"],
 ["fern", "remote"],
 ["drüber", "about it"],
 ["mond", "moon"],
 ["gang", "corridor"],
 ["schauspieler", "actor"],
 ["vollkommen", "completely"],
 ["runde", "round"],
 ["restaurant", "restaurant"],
 ["dreck", "dirt"],
 ["schrieb", "wrote"],
 ["untertitel", "subtitle"],
 ["arthur", "arthur"],
 ["sprichst", "speak"],
 ["warm", "warm"],
 ["mistkerl", "dirty swine"],
 ["nette", "nice"],
 ["bekommst", "get"],
 ["lager", "warehouse"],
 ["atmen", "to breathe"],
 ["legt", "puts"],
 ["daraus", "from that"],
 ["schloss", "lock"],
 ["danny", "danny"],
 ["geschossen", "shot"],
 ["gespräch", "conversation"],
 ["usa", "USA"],
 ["leise", "quietly"],
 ["flughafen", "Airport"],
 ["wunsch", "wish"],
 ["schwanger", "pregnant"],
 ["operation", "surgery"],
 ["anzug", "suit"],
 ["gelegenheit", "opportunity"],
 ["folge", "episode"],
 ["kunden", "Customers"],
 ["dankbar", "grateful"],
 ["welchem", "wherein"],
 ["gewusst", "know"],
 ["tanz", "dance"],
 ["füße", "feet"],
 ["thema", "theme"],
 ["brauchte", "needed"],
 ["dreh", "twist"],
 ["netter", "nice"],
 ["verantwortung", "responsibility"],
 ["umsonst", "for free"],
 ["geändert", "changed"],
 ["subtitles", "subtitles"],
 ["verboten", "forbidden"],
 ["schlechter", "worse"],
 ["million", "million"],
 ["dick", "thick"],
 ["größer", "greater"],
 ["nerven", "annoy"],
 ["schaden", "damage"],
 ["erreicht", "reached"],
 ["jagen", "to hunt"],
 ["urlaub", "vacation"],
 ["erlaubt", "allowed"],
 ["gäste", "Guests"],
 ["erfahrung", "Experience"],
 ["viei", "Viei"],
 ["rot", "red"],
 ["getrunken", "drunk"],
 ["freuen", "looking forward"],
 ["besseres", "better"],
 ["wache", "guard"],
 ["gewalt", "violence"],
 ["entkommen", "escape"],
 ["wußte", "knew"],
 ["hasst", "hates"],
 ["riecht", "smells"],
 ["strand", "Beach"],
 ["sterbe", "die"],
 ["konntest", "could"],
 ["aufstehen", "get up"],
 ["wahnsinnig", "insane"],
 ["nachgedacht", "thought"],
 ["oberst", "colonel"],
 ["william", "william"],
 ["beenden", "break up"],
 ["kino", "movie theater"],
 ["lauter", "volume up"],
 ["folgt", "follows"],
 ["aha", "Aha"],
 ["wahre", "real"],
 ["angebot", "offer"],
 ["erklärt", "explained"],
 ["trink", "drinking"],
 ["soii", "soii"],
 ["stört", "disturbs"],
 ["gilt", "applies"],
 ["keller", "basement, cellar"],
 ["post", "post Office"],
 ["senator", "senator"],
 ["eurem", "your"],
 ["isst", "eats"],
 ["geschichten", "stories"],
 ["frankreich", "France"],
 ["no", "no"],
 ["gentlemen", "gentlemen"],
 ["kugel", "Bullet"],
 ["gemeint", "meant"],
 ["milch", "milk"],
 ["machten", "made"],
 ["kleider", "clothes"],
 ["herausfinden", "find out"],
 ["mindestens", "at least"],
 ["eh", "eh"],
 ["einigen", "some"],
 ["trottel", "Fool"],
 ["schaffst", "manage"],
 ["kuss", "kiss"],
 ["geholfen", "helped"],
 ["knie", "knee"],
 ["worum", "about what"],
 ["grab", "dig"],
 ["voraus", "ahead"],
 ["robin", "robin"],
 ["blind", "blind"],
 ["that", "did"],
 ["schande", "shame"],
 ["carl", "carl"],
 ["schönheit", "beauty"],
 ["versuchte", "tried"],
 ["fuhr", "drove"],
 ["schatten", "shadow"],
 ["schulden", "owe"],
 ["dauern", "last"],
 ["schwimmen", "swim"],
 ["such", "search"],
 ["beruhigen", "soothe"],
 ["form", "shape"],
 ["bessere", "better"],
 ["verschwindet", "disappears"],
 ["zweimal", "twice"],
 ["präsidenten", "president"],
 ["wei", "wei"],
 ["dunkel", "dark"],
 ["idioten", "idiot"],
 ["sieg", "victory"],
 ["riggs", "riggs"],
 ["pläne", "plans"],
 ["bat", "asked"],
 ["zufall", "coincidence"],
 ["eingeladen", "invited"],
 ["scherz", "hoax"],
 ["gezeigt", "shown"],
 ["geheiratet", "got married"],
 ["ed", "ed"],
 ["fahrer", "driver"],
 ["ähnlich", "similar"],
 ["verhalten", "behavior"],
 ["staaten", "states"],
 ["garten", "garden"],
 ["großvater", "grandfather"],
 ["schreibt", "writes"],
 ["pflicht", "mandatory"],
 ["spielte", "played"],
 ["deckung", "cover"],
 ["energie", "energy"],
 ["zeugen", "testify"],
 ["star", "star"],
 ["fährst", "are going"],
 ["nachdenken", "think"],
 ["roy", "roy"],
 ["risiko", "risk"],
 ["geschäfte", "shops"],
 ["erstes", "first"],
 ["kunst", "art"],
 ["behandelt", "treated"],
 ["geschah", "happened"],
 ["helft", "help"],
 ["dürfte", "might"],
 ["heiligen", "hallow"],
 ["mädels", "girls"],
 ["beweis", "proof"],
 ["priester", "priest"],
 ["wusstest", "knew"],
 ["stopp", "stop"],
 ["zustand", "Status"],
 ["direktor", "director"],
 ["gehalten", "held"],
 ["grenze", "border"],
 ["juden", "Jewry"],
 ["geblieben", "remained"],
 ["erzählte", "told"],
 ["vieiieicht", "vieiieicht"],
 ["brennt", "burns"],
 ["interesse", "interest"],
 ["seiten", "pages"],
 
 ['Wandler', 'converter'],
 ['Leistung', 'Performance'],
 ['umwandelt', 'converted'],
 ['herkömmlichen', 'usual'],
 ['erzeugen', 'generate'],
 ['stromdurchflossene', 'current-carrying'],
 ['deren', 'whose'],
 ['gegenseitige', 'mutual'],
 ['Anziehungs', 'attraction'],
 ['Abstoßungskräfte', 'repulsive forces'],
 ['umgesetzt', 'implemented'],
 ['Gegenstück', 'counterpart'],
 ['ähnlich', 'similar'],
 ['Bewegungsleistung', 'Movement performance'],
 ['umwandelt', 'converted'],
 ['meist', 'most of time'],
 ['Bewegungen', 'movements'],
 ['Fahrzeuge', 'vehicles'],
 ['eingesetzt', 'used'],
 ['Inhaltsverzeichnis', 'Table of contents'],
 ['Geschichte', 'Story'],
 ['Grundprinzip', 'Basic principle'],
 ['Begriffe', 'Terms'],
 ['Wechsel', 'Change'],
 ['Stromwender', 'Current inverter'],
 ['Anwendungen', 'Applications'],
 ['Wirkungsgrad', 'Efficiency'],
 ['Effizienz', 'Efficiency'],
 ['Fertigung', 'production'],
 ['Gehäuse', 'Housing'],
 ['Blechpakete', 'Sheet metal packages'],
 ['Welle', 'Wave'],
 ['Endmontage', 'Final assembly'],
 ['Einzelnachweise', 'Individual evidence'],
 ['Geschichte', 'Story'],
 ['Benediktinermönch', 'Benedictine monk'],
 ['erfand', 'invented'],
 ['drehenden', 'rotating'],
 ['Metallstern', 'metal star'],
 ['Entladung', 'discharge'],
 ['dreht', 'rotates'],
 ['Stromversorgung', 'Power supply'],
 ['Erfurt', 'Erfurt'],
 ['Veröffentlichungen', 'Publications'],
 ['Gelehrten', 'scholars'],
 ['beachtet', 'observed'],
 ['verbreitet', 'spread'],
 ['mitunter', 'sometimes'],
 ['jedoch', 'however'],
 ['Erfinder', 'inventor'],
 ['entdeckte', 'discovered'],
 ['Wirkung', 'Effect'],
 ['grundlegendes', 'basic'],
 ['veröffentlichte', 'published'],
 ['Vorrichtung', 'contraption'],
 ['bei', 'at'],
 ['Gegenexperiment', 'Counter experiment'],
 ['beweglicher', 'more mobile'],
 ['festen', 'firm'],
 ['entwickelte', 'developed'],

 ['aufgebauten', 'constructed'],
 ['rotierende', 'rotating'],
 ['Antrieb', 'drive'],
 ['Gerätschaften', 'equipment'],
 ['Arbeitsmaschinen', 'Working machines'],
 ['Gleichstrommotor', 'DC motor'],
 ['Drehstrommotoren', 'Three-phase motors'],
 ['Arten', 'species'],
 ['Drehfeld', 'rotating field'],
 ['Wanderfeld', 'Hiking field'],
 ['Stroms', 'electricity'],
 ['Arbeitsergebnisse', 'work results'],
 ['weiteren', 'further'],
 ['Motorvorläufer', 'engine precursor'],
 ['wirkten', 'worked'],
 ['Weiterentwicklung', 'Further development'],
 ['praxistauglichen', 'practical'],
 ['stattete', 'equipped'],
 ['fassendes', 'comprehensive'],
 ['zwölf', 'twelve'],
 ['entwickelten', 'developed'],
 ['starken', 'strengthen'],
 ['somit', 'thus'],
 ['zugleich', 'at the same time'],
 ['Anwendung', 'Application'],
 ['Praxis', 'Practice'],
 ['darstellte', 'represented'],
 ['erteilt', 'granted'],
 ['Grundlage', 'basis'],
 ['anwendungstauglichen', 'suitable for use'],
 ['entwickelt', 'developed'],
 ['ermöglichte', 'made possible'],
 ['erstmals', 'for the first time'],
 ['Erzeugung', 'generation'],
 ['größerem', 'larger'],
 ['verhalf', 'helped'],
 ['praxistaugliche', 'practical ones'],
 ['weitverbreitete', 'widespread'],
 ['Anwendung', 'Application'],
 ['Entwicklungen', 'developments'],
 ['andersartigen', 'different'],
 ['erlangten', 'acquired'],
 ['zählt', 'counts'],
 ['Dampfmaschine', 'Steam engine'],
 ['aufgebaut', 'built up'],
 ['Kraftrad', 'motorcycle'],
 ['Kraftwerke', 'power plants'],
 ['aufgebaut', 'built up'],
 ['beispielsweise', 'for example'],

 ['Schnittmodell', 'Cutaway model'],
 ['Arbeitsmaschine', 'working machine'],
 ['Umfang', 'Scope'],
 ['Direktantrieb', 'Direct drive'],
 ['ausgeführt', 'executed'],
 ['Antriebe', 'drives'],
 ['häufig', 'frequently'],
 ['Achsen', 'Axles'],
 ['gleichzeitig', 'at the same time'],
 ['bestimmte', 'particular'],
 ['einzelnen', 'individual'],
 ['gewünschte', 'desired'],
 ['Sonstige', 'Other'],
 ['Leiter', 'Director'],
 ['Gleichstrom', 'Direct current'],
 ['Daneben', 'Besides'],
 ['jener', 'that one'],
 ['Bedeutung', 'Meaning'],
 ['wichtigste', 'most important'],
 ['Merkmal', 'characteristic'],
 ['öffentlichen', 'public'],
 ['Pferdebahnen', 'Horse-drawn trams'],
 ['Straßenbahnen', 'Trams'],
 ['ersetzt', 'replaced'],
 ['Gewerbe', 'Business'],
 ['verwendete', 'used'],
 ['anstatt', 'instead of'],
 ['verschiedenster', 'most diverse'],
 ['Magnetfeldpolung', 'Magnetic field polarization'],
 ['Drehbewegung', 'Rotary movement'],
 ['beruht', 'is based'],
 ['mehrere', 'several'],
 ['aufeinander', 'each other'],
 ['ausüben', 'exercise'],
 ['Lorentzkraft', 'Lorentz force'],
 ['üblichen', 'usual'],
 ['feststehenden', 'fixed'],
 ['Außenteil', 'Outdoor part'],
 ['sowie', 'as well as '],
 ['Innenteil', 'inner part'],
 ['besitzt', 'owns'],
 ['Spulen', 'Wash'],
 ['besitzen', 'own'],
 ['erzeugt', 'generated'],
 ['Ausrichtung', 'Alignment'],
 ['abhängig', 'dependent'],
 ['fließt', 'flows'],
 ['entgegengesetzter', 'opposite'],

 ['geringes', 'low'],
 ['Anlaufmoment', 'starting torque'],
 ['benötigen', 'need'],
 ['Anlaufhilfe', 'Start-up aid'],
 ['Wechselstrom', 'Alternating current'],
 ['Wechselspannungen', 'AC voltages'],
 ['Spaltpolmotoren', 'Shaded pole motors'],
 ['Anlauf', 'Start-up'],

 ['Bundesstaat', 'State'],
 ['Basisdaten', 'Basic data'],
 ['Gründung', 'founding'],
 ['Vereinigte', 'United'],
 ['Einwohner', 'Resident'],
 ['Haushalte', 'households'],
 ['Fläche', 'Area'],
 ['Bevölkerungsdichte', 'Population density'],
 ['Höhe', 'Height'],
 ['Postleitzahlen', 'Postal codes'],
 ['Vorwahl', 'Phone prefix'],
 ['Bürgermeister', 'mayor'],
 ['Sehenswürdigkeiten', 'Sightseeing features'],
 ['Verkehr', 'Traffic'],
 ['unmittelbarer', 'more immediate'],
 ['Küstennähe', 'Near the coast'],
 ['grenzt', 'borders'],
 ['südlich', 'southern'],
 ['Volkszählung', 'census'],
 ['verteilten', 'distributed'],
 ['damaligen', 'back then'],
 ['Bevölkerungsdichte', 'Population density'],
 ['Bevölkerung', 'Population'],
 ['bezeichneten', 'designated'],
 ['gaben', 'gifts'],
 ['Ethnie', 'Ethnicity'],
 ['mehreren', 'several'],
 ['Bevölkerung', 'Population'],
 ['bestand', 'duration'],

 ['Einwohnern', 'residents'],
 ['bestehend', 'existing'],
 ['verheirateten', 'married'],
 ['Nachkommen', 'progeny'],
 ['Elternteil', 'parent'],
 ['Nachkomme', 'descendant'],
 ['durchschnittliche', 'average'],
 ['Größe', 'Size'],
 ['Haushalts', 'household'],
 ['jünger', 'younger'],
 ['mittlere', 'medium'],
 ['männlich', 'masculine'],
 ['weiblich', 'female'],
 ['Armutsgrenze', 'poverty line'],
 ['Muttersprache', 'mother tongue'],
 ['gelistet', 'listed'],
 ['Stadtgebiet', 'urban area'],
 ['durchquert', 'crossed'],
 ['tangiert', 'affected'],
 ['Güterbahnstrecke', 'Freight railway line'],
 ['abgerufen', 'retrieved'],
 ['Sprachverteilung', 'Language distribution'],
 ['angemeldet', 'Registered'],
 ['Beiträge', 'Posts'],
 ['Benutzerkonto', 'user account'],
 ['erstellen', 'create'],
 ['Anmelden', 'Register'],
 ['bearbeitenVersionsgeschichteSuche', 'editversion historysearch'],
 ['durchsuchen', 'search'],
 ['Zufälliger', 'Random'],
 ['Mitmachen', 'Participate'],
 ['verbessern', 'improve'],
 ['anlegen', 'invest'],
 ['Änderungen', 'Changes'],
 ['Spenden', 'Donate'],
 ['Werkzeuge', 'Tools'],
 ['verlinkten', 'linked'],

 ['Bedeutungen', 'Meanings'],
 ['siehe', 'please refer'],
 ['Begriffsklärung', 'Disambiguation'],
 ['Staatsmotto', 'State motto'],
 ['Amtssprache', 'Official language'],
 ['Mitglied', 'Member'],
 ['März', 'March'],
 ['Höchster', 'Supreme'],
 ['Durchschn', 'Avg'],
 ['Tiefster', 'Deepest'],
 ['Aussprache', 'Pronunciation'],
 ['abspielen', 'play'],
 ['Südosten', 'southeast'],
 ['Vereinigten', 'United'],
 ['Sonnenscheinstaat', 'Sunshine State'],
 ['drittbevölkerungsreichste', 'third most populous'],
 ['dichtesten', 'densest'],
 ['besiedelten', 'populated'],
 ['gleichnamige', 'same name'],
 ['Halbinsel', 'peninsula'],
 ['Entdeckern', 'explorers'],
 ['entdeckt', 'discovered'],
 ['benannt', 'named'],
 ['Pascua', 'Pascua'],
 ['Hauptstadt', 'Capital city'],
 ['Ausdehnung', 'expansion'],
 ['Staatsgebiets', 'national territory'],
 ['Nachbarstaaten', 'Neighbore states'],
 ['Klima', 'climate'],
 ['Städte', 'Cities'],
 ['Vorgeschichte', 'prehistory'],
 ['Hugenottische', 'Huguenot tables'],
 ['Siedlungen', 'settlements'],
 ['Feindseligkeiten', 'hostilities'],
 ['Herrschaft', 'Domination'],
 ['indigenen', 'indigenous'],
 ['Erneut', 'Anew'],
 ['spanischer', 'Spanish'],
 ['Eroberung', 'conquest'],
 ['Staatsregierung', 'State government'],
 ['Bundesstaatliche', 'Federal'],
 ['Gliederung', 'outline'],
 ['Mitglieder', 'members'],
 ['Todesstrafe', 'death penalty'],
 ['Abtreibungsverbot', 'Abortion ban'],
 ['Naturdenkmäler', 'Natural monuments'],
 ['Wirtschaft', 'Business'],
 ['Sonstiges', 'Miscellaneous'],
 ['Halbinsel', 'peninsula'],
 ['Festlandteil', 'mainland part'],
 ['Inselkette', 'Island chain'],
 ['bekanntesten', 'most famous'],
 ['Brücken', 'bridges'],
 ['verbunden', 'tied together'],
 ['befindet', 'located'],
 ['Staatsgebietes', 'national territory'],
 ['Wasserflächen', 'water areas'],
 ['Ausdehnung', 'expansion'],
 ['Staatsgebiets', 'national territory'],
 ['Breite', 'Width'],
 ['Nachbarstaaten', 'Neighbore states'],
 ['Bundesstaaten', 'States'],

 ['bevölkerungsreichste', 'most populous'],
 ['Ostküste', 'East Coast'],
 ['südlichen', 'southern'],
 ['südlichste', 'southernmost'],
 ['Gesamtfläche', 'total area'],
 ['Ausbuchtung', 'bulge'],
 ['übermeerische', 'oversea'],
 ['Untergrund', 'underground'],
 ['zusammengesetzt', 'composed'],
 ['präkambrischem', 'precambrian'],
 ['Vulkangestein', 'Volcanic rock'],
 ['späteren', 'later'],
 ['vulkanischen', 'volcanic'],
 ['Gesteinen', 'rocks'],
 ['Strukturprägend', 'Structural'],
 ['Kalkstein', 'limestone'],
 ['überwiegend', 'mostly'],
 ['porös', 'porous'],
 ['porösen', 'porous'],

 ['Sprecher', 'speaker'],
 ['Indogermanische', 'Indo-European'],
 ['anhören', 'listen'],
 ['verschiedenen', 'different'],
 ['Einwanderer', 'immigrant'],
 ['beeinflusst', 'influenced'],
 ['Ureinwohnern', 'natives'],
 ['auffälligste', 'most noticeable'],
 ['Vokalen', 'vowels'],
 ['Rhotizität', 'Rhoticity'],
 ['Wortende', 'end of word'],
 ['ausgesprochen', 'pronounced'],
 ['Besonderheiten', 'particularities'],
 ['Wortebene', 'word level'],
 ['Vergleich', 'Comparison'],
 ['Unterschiede', 'differences'],
 ['bestehen', 'consist'],
 ['Wichtige', 'Important'],
 ['bezeichnet', 'designated'],
 ['umfangreiche', 'extensive'],
 ['Forschung', 'Research'],
 ['darunter', 'underneath'],
 ['Wörterbücher', 'Dictionaries'],
 ['Verteilung', 'distribution'],
 ['Ungespannt', 'Relaxed'],
 ['Wortschatz', 'vocabulary'],
 ['Unterschiede', 'differences'],
 ['Forschung', 'Research'],
 ['Allgemeine', 'General'],
 ['Beschreibungen', 'Descriptions'],
 ['sogenannte', 'so-called'],
 ['westgermanischen', 'West Germanic'],
 ['plurizentrischen', 'pluricentric'],
 ['bezeichnet', 'designated'],
 ['Mehrheit', 'majority'],
 ['erhebliche', 'significant'],
 ['Minderheiten', 'Minorities'],
 ['Zweitsprache', 'second language'],
 ['erwerben', 'acquire'],
 ['indische', 'Indian'],
 ['Amtssprachen', 'Official languages'],
 ['Bildungswesen', 'Education'],
 ['mehrheitlich', 'majority'],
 ['Sprecher', 'speaker'],
 ['Hauptvarietät', 'main variety'],
 ['Ländern', 'countries'],

 ['Weiterleitung', 'Forwarding'],
 ['aufgeführt', 'listed'],
 ['Nebensprache', 'secondary language'],
 ['Handels', 'commercial'],
 ['Verkehrssprache', 'lingua franca'],
 ['Milliarden', 'billion'],
 ['Eigenbezeichnung', 'Proper designation'],
 ['ursprünglich', 'originally'],
 ['beheimatete', 'homed'],
 ['Zweig', 'branch'],
 ['frühen', 'early'],
 ['Mittelalter', 'middle Ages'],
 ['Einwanderung', 'immigration'],
 ['Völker', 'people'],
 ['Angeln', 'fishing'],
 ['herleitet', 'derives'],
 ['Sachsen', 'Saxony'],
 ['Frühformen', 'Early forms'],
 ['verwandten', 'relatives'],
 ['lebenden', 'living'],
 ['friesischen', 'Frisian'],
 ['Niederdeutsche', 'Low German'],
 ['Festland', 'Mainland'],
 ['Verlauf', 'Course'],
 ['starke', 'strength'],
 ['Sonderentwicklungen', 'Special developments'],
 ['ausgebildet', 'educated'],
 ['Satzbau', 'Sentence structure'],
 ['wechselte', 'changed'],
 ['Gegensatz', 'contrast'],
 ['Verwandten', 'relatives'],
 ['verlor', 'lost'],
 ['Verbzweiteigenschaft', 'verb second property'],
 ['Bildung', 'Education'],
 ['Wortformen', 'Word forms'],
 ['Substantiven', 'nouns'],
 ['abgebaut', 'reduced'],
 ['frühen', 'early'],
 ['zunächst', 'first'],
 ['zeitweilige', 'temporary'],
 ['Besetzung', 'occupation'],
 ['Jahrhundert', 'century'],
 ['ergab', 'resulted'],
 ['nochmals', 'again'],
 ['Prägung', 'Embossing'],
 ['aufgrund', 'because of'],
 ['vielfältigen', 'diverse'],
 ['Einflüsse', 'Influences'],
 ['heutige', 'today'],
 ['außergewöhnlich', 'unusual'],
 ['umfangreichen', 'extensive'],
 ['wesentliche', 'essential'],
 ['Fixierung', 'Fixation'],
 ['Rechtschreibung', 'spelling, orthography'],
 ['erfolgte', 'took place'],
 ['Aufkommen', 'Come up'],
 ['Buchdrucks', 'letterpress'],
 ['trotz', 'despite'],
 ['fortlaufenden', 'ongoing'],
 ['Lautwandels', 'sound change'],
 ['Abbildung', 'Illustration'],
 ['tatsächlichen', 'actual'],
 ['Lautgestalt', 'sound shape'],
 ['vielfältig', 'diverse'],
 ['abweicht', 'differs'],
 ['Ausgehend', 'Outgoing'],
 ['Entstehungsort', 'Place of origin'],
 ['breitete', 'spread'],
 ['gesamten', 'entire'],
 ['Inseln', 'Islands'],
 ['verdrängte', 'repressed'],
 ['allmählich', 'gradually'],
 ['gesprochenen', 'spoken'],
 ['kleinere', 'smaller'],
 ['Sprechergemeinschaften', 'Speaker communities'],
 ['inmitten', 'midst'],
 ['Sprachraums', 'language area'],
 ['fortbestehen', 'persist'],
 ['infolge', 'as a result'],
 ['meistgesprochenen', 'most spoken'],
 ['Länder', 'Countries'],
 ['Gebiete', 'areas'],
 ['ehemalige', 'former'],
 ['Bewohner', 'residents'],
 ['Fremdsprache', 'foreign language'],
 ['gelehrt', 'taught'],
 ['offizielle', 'official'],
 ['wobei', 'where'],
 ['verständigten', 'communicated'],
 ['Westdeutschland', 'West Germany'],
 ['Abkommen', 'agreement'],
 ['Schulen', 'schools'],
 ['generell', 'generally'],
 ['Pflichtfremdsprache', 'Compulsory foreign language'],
 ['einzuführen', 'to introduce'],

 ['hansa', 'Hansa'],
 ['aufsicht', 'at sight'],
 ['angetreten', 'started'],
 ['üben', 'practice'],
 ['versprach', 'promised'],
 ['rauball', 'roughball'],
 ['gewinsel', 'whine'],
 ['kapieren', 'get it'],
 ['merkel', 'Merkel'],
 ['einig', 'Some'],
 ['beerdigung', 'burial'],
 ['oberlauterbach', 'Oberlauterbach'],
 ['todesfall', 'death'],
 ['untersuchen', 'investigate'],
 ['grausamste', 'cruelest'],
 ['geistlich', 'spiritually'],
 ['treppe', 'stairs'],
 ['gestolpert', 'stumbled'],
 ['stillstehen', 'stand still'],
 ['eingang', 'entrance'],
 ['kriegs', 'war'],
 ['dienen', 'serve'],
 ['wahrer', 'truer'],
 ['diener', 'servant'],
 ['volkes', 'people'],
 ['selbstaufopferung', 'self-sacrifice'],
 ['entsprochen', 'corresponded'],
 ['fähnriche', 'ensigns'],
 ['vorbild', 'example'],
 ['genutzt', 'used'],
 ['verschafft', 'provided'],
 ['verwundet', 'wounded'],
 ['abzeichen', 'badge'],
 ['meldung', 'report'],
 ['oberleutnants', 'first lieutenants'],
 ['marsch', 'march'],
 ['gesetzt', 'set'],
 ['durchsuchung', 'search'],
 ['halts', 'wait'],
 ['beauftragt', 'instructed'],
 ['begrüße', 'welcome'],
 ['untersuchung', 'investigation'],
 ['sinne', 'senses'],
 ['gründlichste', 'most thorough'],
 ['vorzugehen', 'to proceed'],
 ['schweinerei', 'mess'],
 ['pfarrern', 'pastors'],
 ['armbruch', 'broken arm'],
 ['aussuchen', 'choose'],
 ['nasen', 'noses'],
 ['telefonzentrale', 'switchboard'],
 ['helferinnen', 'helpers'],
 ['größerer', 'bigger'],
 ['dauer', 'duration'],
 ['aufsichtsführenden', 'supervisory'],
 ['meldung', 'report'],
 ['erstattet', 'reimbursed'],
 ['täter', 'perpetrator'],
 ['angeblich', 'allegedly'],
 ['belästigt', 'harassed'],
 ['mainz', 'mainz'],
 ['stamm', 'tribe'],
 ['kotten', 'Kotten'],
 ['rhein', 'Rhine'],
 ['erleben', 'experience'],
 ['eisigen', 'icy'],
 ['geflogen', 'flown'],
 ['schweinerei', 'mess'],
 ['bedienen', 'serve'],
 ['zentrale', 'central'],
 ['stellung', 'position'],
 ['reparieren', 'repair'],
 ['hergefallen', 'fallen'],
 ['gestürzt', 'fell'],
 ['ausgezogen', 'moved out'],
 ['gewehrt', 'defended'],
 ['wild', 'wild'],
 ['vergewaltigt', 'raped'],
 ['originell', 'original'],
 ['andersrum', 'the other way around'],
 ['abwechslung', 'variety'],
 ['erfährt', 'learns'],
 ['kieker', 'kieker'],
 ['vertuschen', 'cover up'],
 ['kotten', 'Kotten'],
 ['behauptet', 'claims'],
 ['gestürzt', 'fell'],
 ['ausgezogen', 'moved out'],
 ['sternhagen', 'sternhagen'],
 ['vermittlung', 'mediation'],
 ['zog', 'pulled'],
 ['stiefel', 'boots'],
 ['außen', 'Outside'],
 ['schmissen', 'threw'],
 ['harke', 'rake'],
 ['zogen', 'pulled'],
 ['unterhosen', 'underpants'],
 ['klamotten', 'clothes'],
 ['landete', 'landed'],
 ['aufsicht', 'at sight'],
 ['krach', 'crash'],
 ['groben', 'coarse'],
 ['unfugs', 'nonsense'],
 ['oberleitner', 'Oberleitner'],
 ['waadt', 'Vaud'],
 ['gespräche', 'conversations'],
 ['bekanntschaften', 'acquaintances'],
 ['auszugehen', 'to go out'],
 ['verfolgen', 'track'],
 ['belästigen', 'harass'],
 ['ausgerechnet', 'of all things'],
 ['ehrenvolle', 'honorable'],
 ['kernel', 'kernel'],
 ['erhört', 'he is listening'],
 ['mittag', 'midday'],
 ['menschenschinder', 'human abuser'],
 ['gespannt', 'tense'],
 ['pfeifen', 'Pipes'],
 ['verrechnen', 'calculate'],
 ['vergangen', 'past'],
 ['wirklicher', 'more real'],
 ['frühere', 'earlier'],
 ['oberfeld', 'Oberfeld'],
 ['würmern', 'worms'],
 ['schnüffler', 'snoop'],
 ['schnüffeln', 'sniff'],
 ['auffallen', 'stand out'],
 ['druckkosten', 'printing costs'],
 ['riskieren', 'take risk'],
 ['oberländern', 'Oberlander'],
 
 ['schuldendienst', 'debt service'],
 ['stundenplan', 'Timetable'],
 ['gänse', 'geese'],
 ['mauern', 'wall up'],
 ['gruß', 'greeting'],
 ['rauscht', 'rushes'],
 ['sinnlose', 'pointless'],
 ['kraftprobe', 'test of strength'],
 ['stürmen', 'storm'],
 ['abrücken', 'move away'],
 ['staatspolizei', 'state police'],
 ['geheimen', 'secret'],
 ['vornamen', 'first names'],
 ['egon', 'egon'],
 ['geführt', 'guided'],
 ['pauschalen', 'flat rates'],
 ['vornamen', 'first names'],
 ['heinrich', 'Heinrich'],
 ['entscheidet', 'decides'],
 ['egon', 'egon'],
 ['aufnahme', 'recording'],
 ['foul', 'foul'],
 ['unterzeichnet', 'signed'],
 ['vorn', 'front'],
 ['ehrliche', 'honest'],
 ['zähne', 'teeth'],
 ['küchen', 'kitchens'],
 ['mullen', 'mullen'],
 ['bundle', 'bundle'],
 ['leuchten', 'to shine'],
 ['verschossen', 'missed'],
 ['klappt', 'works'],
 ['schnaps', 'liquor'],
 ['prost', 'Bottom up'],
 ['lieblingslied', 'favourite song'],
 ['ausführliche', 'detailed'],
 ['ansprache', 'speech'],
 ['meinetwegen', 'because of me'],
 ['rauschen', 'rush'],
 ['geklaut', 'Stolen'],
 ['erlaubst', 'allow'],
 ['freier', 'freer'],
 ['offizieren', 'officers'],
 ['sterne', 'Stars'],
 ['widerling', 'disgusting'],
 ['großmutter', 'grandmother'],
 ['versorger', 'provider'],
 ['tritt', 'occurs'],
 ['holz', 'wood'],
 ['mitternacht', 'midnight'],
 ['kaserne', 'barracks'],
 ['besondere', 'special'],
 ['vorkommnisse', 'incidents'],
 ['gemütliche', 'cozy'],
 ['steuerfrei', 'tax free'],
 ['tassen', 'cups'],
 ['verabredung', 'appointment'],
 ['hauptmann', 'captain'],
 ['entscheidest', 'you decide'],
 ['bemerkung', 'remark'],
 ['harmlose', 'harmless'],
 ['niederträchtige', 'vile ones'],
 ['gemeinheit', 'meanness'],
 ['baue', 'build'],
 ['beleidigung', 'insult'],
 ['scherben', 'shards'],
 ['sticheleien', 'taunts'],
 ['täuscht', 'deceiving'],
 ['aufhört', 'stops'],
 ['entschuldigt', 'sorry'],
 ['platzieren', 'place'],
 ['gemütliche', 'cozy'],
 ['finster', 'dark'],
 ['stimmung', 'mood'],
 ['teuer', 'expensive'],
 ['schreibstube', 'writing room'],
 ['leckereien', 'treats'],
 ['drücker', 'pusher'],
 ['sichs', 'yourself'],
 ['sitze', 'sit'],
 ['weber', 'Weber'],
 ['schrecken', 'scare'],
 ['abbaus', 'degradation'],
 ['angsträume', 'anxiety dreams'],
 ['stehst', 'stand'],
 ['persönlichen', 'personal'],
 ['schutz', 'protection'],
 ['heiliger', 'holier'],
 ['entzündeten', 'ignited'],
 ['prügel', 'spanking'],
 ['kasse', 'checkout'],
 ['armleuchter', 'chandelier'],
 ['trinkt', 'drinks'],
 ['schnaps', 'liquor'],
 ['hohen', 'high'],
 ['stangen', 'rods'],
 ['bedient', 'served'],
 ['lehrgangs', 'course'],
 ['ganoven', 'crooks'],
 ['abrechnen', 'settle up'],
 ['küsschen', 'Peck'],
 ['ungelegen', 'inconvenient'],
 ['wäscht', 'washes'],
 ['kindchen', 'child'],
 ['dringendes', 'urgent'],
 ['bequem', 'comfortable'],
 ['schulte', 'trained'],
 ['untergang', 'downfall'],
 ['neuester', 'newest'],
 ['kameradschaftliche', 'comradely'],
 ['weber', 'Weber'],
 ['beendet', 'completed'],
 ['streit', 'dispute'],
 ['inzwischen', 'in the meantime'],
 ['schnauze', 'snout'],
 ['erledigten', 'done'],
 ['aussagen', 'testify'],
 ['gefasst', 'caught'],
 ['hauptmann', 'captain'],
 ['blaue', 'blueness'],
 ['angeschlagene', 'battered'],
 ['nasenbein', 'nasal bone'],
 ['geruhsamen', 'peace and quiet'],
 ['garnisonkirche', 'garrison church'],
 ['sonntag', 'Sunday'],
 ['trat', 'stepped'],
 ['exerzieren', 'drill'],
 ['fähnrich', 'ensign'],
 ['schleichen', 'to sneak'],
 ['helden', 'heroes'],
 ['bunten', 'colorful'],
 ['kocht', 'cooks'],
 ['übung', 'exercise'],
 ['wolle', 'wool'],
 ['soeben', 'just'],
 ['anzeige', 'advertisement'],
 ['angekommen', 'arrived'],
 ['briefträger', 'postman'],
 ['katers', 'hangover'],

 ['unangenehm', 'unpleasant'],
 ['aufgefallen', 'noticed'],
 ['befreien', 'to free'],
 ['abgeschoben', 'deported'],
 ['oberleutnant', 'first lieutenant'],
 ['fähigkeiten', 'skills'],
 ['klicks', 'clicks'],
 ['kommandiert', 'commanded'],
 ['fähigkeiten', 'skills'],
 ['kommentierung', 'comment'],
 ['ausschlaggebend', 'crucial'],
 ['sogenannter', 'so-called'],
 ['unbequemer', 'more uncomfortable'],
 ['untergebener', 'subordinate'],
 ['abgeschoben', 'deported'],
 ['personalakte', 'personnel file'],
 ['entnommen', 'removed'],
 ['früheren', 'previous'],
 ['kommentiert', 'commented'],
 ['differenzen', 'differences'],
 ['erstattete', 'refunded'],
 ['veruntreuung', 'embezzlement'],
 ['vorgesetzen', 'superior'],
 ['betrüger', 'cheater'],
 ['mängel', 'defects'],
 ['vorschrift', 'regulation'],
 ['rechtzeitig', 'punctual'],
 ['gemeldet', 'reported'],
 ['gelungen', 'succeeded'],
 ['übergeben', 'hand over'],
 ['dienstgeschäfte', 'official business'],
 ['ernenne', 'appoint'],
 ['heutigen', 'today'],
 ['aufsichtsrat', 'supervisory board'],
 ['absicht', 'intention'],
 ['erlauben', 'allow'],
 ['geeignet', 'suitable'],
 ['zweifeln', 'doubt'],
 ['urteilsfähigkeit', 'ability to judge'],
 ['oberleutnant', 'first lieutenant'],
 ['bakugan', 'bakugan'],
 ['tellermine', 'Tellermine'],
 ['gesprengt', 'blown up'],
 ['rechtzeitig', 'punctual'],
 ['zerfetzt', 'shredded'],
 ['zeitlang', 'for a while'],
 ['ostfrauen', 'eastern women'],
 ['sprengungen', 'blasts'],
 ['durchgeführt', 'carried out'],
 ['zwischenfälle', 'incidents'],
 ['verdacht', 'suspected'],
 ['bestätigen', 'confirm'],
 ['entkräften', 'invalidate'],
 ['offiziellen', 'official'],
 ['untersuchungen', 'investigations'],
 ['verlaufen', 'get lost'],
 ['selbstständig', 'self-employed'],
 ['denkenden', 'thinking'],
 ['herausfindet', 'finds out'],
 ['übergebe', 'hand over'],
 ['schriftlichen', 'written'],
 ['nachlass', 'estate'],
 ['corot', 'corot'],
 ['ambako', 'ambako'],
 ['fingerzeig', 'finger point'],
 ['fronterfahrung', 'frontline experience'],
 ['gewaltmaßnahmen', 'violent measures'],
 ['tägliches', 'daily'],
 ['wenden', 'turn around'],
 ['verhältnisse', 'relationships'],
 ['seitenlinie', 'sideline'],
 ['häufiger', 'more often'],
 ['frontsoldaten', 'frontline soldiers'],
 ['aufsichtsräten', 'supervisory boards'],
 ['sprossen', 'sprouted'],
 ['gewöhnlicher', 'more ordinary'],
 ['gemütliches', 'cozy'],
 ['beschränkt', 'limited'],
 ['ausgeschlossen', 'excluded'],
 ['bemerkungen', 'Remarks'],
 ['unangebracht', 'inappropriate'],
 ['zackig', 'jagged'],
 ['zündschnur', 'fuse'],
 ['rechtlichen', 'legal'],
 ['anspielungen', 'allusions'],
 ['hinzuhören', 'listen'],
 ['unterlassen', 'refrain from'],
 ['wartest', 'wait'],
 ['anwächst', 'grows'],
 ['hämische', 'malicious'],
 ['aufsichts', 'supervisory'],
 ['oberleutnant', 'first lieutenant'],
 ['überzeugt', 'convinced'],
 ['entgegenbringen', 'to accommodate'],
 ['erläutert', 'explained'],
 ['hinsetzen', 'sit down'],
 ['stettin', 'Stettin'],
 ['rendant', 'rendant'],
 ['eingezogen', 'moved in'],
 ['wetterwechsel', 'weather change'],
 ['spüre', 'feel'],
 ['aufsichts', 'supervisory'],
 ['älteste', 'oldest'],
 ['berufsunteroffizier', 'career sergeant'],
 ['bauer', 'Farmer'],
 ['schneidermeisterin', 'master tailor'],
 ['ortsgruppenleiter', 'local group leader'],
 ['hauptamtliche', 'full-time'],
 ['hitlerjugend', 'Hitler Youth'],
 ['führer', 'leader'],
 ['freiwillig', 'voluntarily'],
 ['gemeldet', 'reported'],
 ['ostfront', 'eastern front'],
 ['gekämpft', 'fought'],
 ['bäckermeister', 'master baker'],
 ['werdau', 'werdau'],
 ['bäckerhandwerk', 'bakery craft'],
 ['erlernt', 'he learns'],
 ['beruf', 'profession'],
 ['abitur', 'high school diploma'],
 ['eingezogen', 'moved in'],
 ['anderthalb', 'one and a half'],
 ['dienste', 'services'],
 ['bluten', 'blossoms'],
 ['nasenbluten', 'nosebleeds'],
 ['meistens', 'mostly'],
 ['schach', 'chess'],
 ['nachfolgers', 'successor'],
 ['ernennung', 'appointment'],
 ['überraschend', 'surprised'],
 ['eingeführt', 'introduced'],
 ['merkwürdige', 'strange'],
 ['geschliffenen', 'ground'],
 ['konsterniert', 'consternated'],
 ['kindermädchen', 'nanny'],
 ['hameln', 'hameln'],
 ['ambitionen', 'ambitions'],
 ['bock', 'desire'],
 ['verbringt', 'spends'],
 ['laufenden', 'ongoing'],
 ['lehrgang', 'course'],
 ['beträgt', 'amounts'],
 ['erfüllt', 'Fulfills'],
 ['ausschuss', 'committee'],
 ['liefern', 'delivery'],
 ['grunde', 'reasons'],
 ['hameln', 'hameln'],
 ['beibringen', 'teach'],
 ['landes', 'country'],
 ['befolgt', 'followed'],
 ['niederträchtig', 'vile'],
 ['gewinnt', 'wins'],
 ['kratzbürstig', 'prickly'],
 ['undankbar', 'ungrateful'],
 ['versorgen', 'take care of'],
 ['hauen', 'hit'],
 ['reizende', 'lovely'],
 ['hattersheim', 'hattersheim'],
 ['tröpfchen', 'droplet'],
 ['gelingen', 'succeed'],
 ['anstoßen', 'nudge'],
 ['kohl', 'cabbage'],
 ['kühl', 'cool'],
 ['gläsern', 'glasses'],
 ['offiziere', 'officers'],
 ['beschweren', 'complain'],
 ['verpflegung', 'catering'],
 ['erhält', 'receives'],
 ['eingebrochen', 'broken into'],
 ['üblich', 'usual'],
 ['decken', 'cover'],
 ['überprüfung', 'examination'],
 ['qualität', 'quality'],
 ['schreibmaschine', 'typewriter'],
 ['lerne', 'learn'],
 ['beibringen', 'teach'],
 ['schwindler', 'fraudster'],
 ['schach', 'chess'],
 ['partien', 'games'],
 ['träumt', 'dreams'],
 ['sommernachtstraum', "Midsummer Night's Dream"],
 ['vallotton', 'Vallotton'],
 ['handkraft', 'hand strength'],
 ['klagt', 'complains'],
 ['erschreckende', 'frightening'],
 ['schauerlichen', 'gruesome'],
 ['katona', 'Katona'],
 ['möblierten', 'furnished'],
 ['kleinigkeit', 'small thing'],
 ['ecken', 'corners'],
 ['pflichtbewusste', 'dutiful ones'],
 ['zuviel', 'too much'],
 ['verabschieden', 'say goodbye'],
 ['gemachter', 'made'],
 ['stoppte', 'stopped'],
 ['stecke', 'stuck'],
 ['sand', 'sand'],
 ['beleidigt', 'insulted'],
 ['gestatten', 'allow'],
 ['gestattet', 'permitted'],
 ['boshaft', 'malicious'],
 ['ohr', 'ear'],
 ['flüstern', 'to whisper'],
 ['kaders', 'squad'],
 ['granatsplitter', 'shrapnel'],
 ['männlichen', 'male'],
 ['herrlichkeit', 'glory'],
 ['müssten', 'would have to'],
 ['betreut', 'cared for'],
 ['standpunkt', 'position'],
 ['undurchsichtigen', 'opaque'],
 ['eingekauft', 'purchased'],
 ['durchschauen', 'see through'],
 ['heil', 'unhurt'],
 ['herauskommen', 'come out'],
 ['rechnung', 'invoice'],
 ['wirt', 'host'],
 ['wechseln', 'change'],
 ['hameln', 'hameln'],
 ['windeln', 'diapers'],
 ['zahl', 'number'],
 ['gründlich', 'thorough'],
 ['bereuen', 'regret'],
 ['regnet', 'raining'],
 ['lehrgang', 'course'],
 ['aufmacht', 'opens'],
 ['kramer', 'chandler'],
 ['dämlichen', 'stupid'],
 ['schnarchen', 'snoring'],
 ['betten', 'beds'],
 ['drama', 'drama'],
 ['erschüttert', 'shocked'],
 ['zahlencode', 'number code'],
 ['kerzen', 'candles'],
 ['älteren', 'older'],
 ['geschwister', 'siblings'],
 ['münchen', 'Munich'],
 ['fliegerbombe', 'aircraft bomb'],
 ['nachzug', 'follow-up'],
 ['ballack', 'ballack'],
 ['ratsherr', 'councillor'],
 ['gehorsams', 'obedient'],
 ['einladung', 'invitation'],
 ['hofbauer', 'farmer'],
 ['verglich', 'compared'],
 ['sehnlichster', 'most eagerly'],
 ['lehrgang', 'course'],
 ['bestimmen', 'determine'],
 ['beobachtet', 'observed'],
 ['offizier', 'officer'],
 ['ideale', 'ideal'],
 ['anstoßen', 'nudge'],
 ['verbinden', 'connect'],
 ['dringe', 'penetrate'],
 ['ratsherr', 'councillor'],
 ['bewilligung', 'approval'],
 ['vorbehalten', 'Reserved'],
 ['felicitas', 'felicitas'],
 ['handelt', 'acts'],
 ['namensgebung', 'naming'],
 ['scheußlich', 'awful'],
 ['änderung', 'the change'],
 ['vielmehr', 'much more'],
 ['klingenden', 'sounding'],
 ['ausgerottet', 'eradicated'],
 ['gesperrt', 'blocked'],
 ['befugt', 'authorized'],
 ['ausnahme', 'exception'],
 ['zahlreiche', 'numerous'],
 ['truppen', 'troops'],
 ['abgesetzte', 'discontinued'],
 ['luftlande', 'airborne'],
 ['verbände', 'associations'],
 ['landungen', 'landings'],
 ['unterstützen', 'support'],
 ['heranführen', 'introduce'],
 ['reserven', 'reserves'],
 ['verhindern', 'impede'],
 ['apparat', 'apparatus'],
 ['modersohn', 'modersohn'],
 ['oberleutnant', 'first lieutenant'],
 ['apparat', 'apparatus'],
 ['dringend', 'urgent'],
 ['kersten', 'kersten'],
 ['hollenbach', 'Hollenbach'],
 ['mandanten', 'clients'],
 ['herausgegeben', 'published'],
 ['wehrmachts', 'wehrmachts'],
 ['anweisung', 'instruction'],
 ['erlassen', 'enacted'],
 ['abgeschlossene', 'completed'],
 ['kampfhandlungen', 'combat operations'],
 ['besprochen', 'discussed'],
 ['klamm', 'clammy'],
 ['verhandlung', 'negotiation'],
 ['abgeschlossenen', 'completed'],
 ['erfahrungen', 'experiences'],
 ['strikte', 'strict'],
 ['anwesen', 'estate'],
 ['trage', 'sluggish'],
 ['gegner', 'opponent'],
 ['wehrmachts', 'wehrmachts'],
 ['wehrmacht', 'Wehrmacht'],
 ['berichtigt', 'corrected'],
 ['maßgebende', 'authoritative'],
 ['verlautbarung', 'announcement'],
 ['oberkommandos', 'high command'],
 ['wehrmacht', 'Wehrmacht'],
 ['fronten', 'fronts'],
 ['bibel', 'Bible'],
 ['hajo', 'haha'],
 ['hoffmann', 'hoffman'],
 ['absatz', 'paragraph'],
 ['gestrigen', 'yesterday'],
 ['brettes', 'board'],

 ['akw', 'ak'],
 ['jacke', 'jacket'],
 ['appetit', 'appetite'],
 ['skandalöse', 'scandalous'],
 ['zustände', 'conditions'],
 ['ermittelt', 'determined'],
 ['kompanie', 'company'],
 ['auskünfte', 'information'],
 ['kater', 'Male cat'],
 ['ermittelt', 'determined'],
 ['zuständig', 'responsible'],
 ['bewahre', 'beware'],
 ['dienstweg', 'official route'],
 ['einhalten', 'retain'],
 ['auskunft', 'information'],
 ['verlange', 'demand'],
 ['couragierte', 'courageous'],
 ['widerpart', 'contradicted'],
 ['behringer', 'behringer'],
 ['echten', 'real'],
 ['linken', 'left'],
 ['anschluss', 'connection'],
 ['anlass', 'occasion'],
 ['kürzlich', 'recently'],
 ['erfolgten', 'took place'],
 ['luftangriffen', 'air raids'],
 ['theoretische', 'theoretical'],
 ['durchführen', 'carry out'],
 ['großfeuer', 'big fire'],
 ['unterhaltung', 'entertainment'],
 ['feuerwehr', 'fire department'],
 ['theoretischen', 'theoretical'],
 ['flammenmeer', 'sea of flames'],
 ['schmoren', 'braise'],
 ['spanferkel', 'suckling pig'],
 ['ausdrücke', 'expressions'],
 ['fehle', 'missing'],
 ['angerichtet', 'arranged'],
 ['kennenlernen', 'get to know'],
 ['höchstes', 'highest'],
 ['schaufenster', 'Store window'],
 ['interessanter', 'more interesting'],
 ['gewisse', 'certain'],
 ['ansprechen', 'address'],
 ['erlebt', 'experienced'],
 ['tollsten', 'most amazing'],
 ['langeweile', 'boredom'],
 ['einlässt', 'lets in'],
 ['hinterher', 'after'],
 ['sammeln', 'collect'],
 ['lohnen', 'worthwhile'],
 ['richters', 'judge'],
 ['abgelehnt', 'rejected'],
 ['lediglich', 'only '],
 ['dienstweg', 'official route'],
 ['verwiesen', 'referred'],
 ['behandlung', 'treatment'],
 ['kinderei', 'childishness'],
 ['vorliegt', 'is present'],
 ['juristisch', 'legal'],
 ['beurteilt', 'assessed'],
 ['verlässt', 'leaves'],
 ['disziplinarisch', 'disciplinary'],
 ['kompanie', 'company'],
 ['ebene', 'level'],
 ['mücke', 'mosquito'],
 ['elefanten', 'elephants'],
 ['veranlassen', 'cause'],
 ['fummeln', 'fumble'],
 ['aktien', 'shares'],
 ['manches', 'some things'],
 ['verbogen', 'bent'],
 ['dienstagmorgen', 'Tuesday morning'],
 ['menschlicher', 'more human'],
 ['derselbe', 'same'],
 ['wecken', 'wake up'],
 ['frühsport', 'early exercise'],
 ['dienstgeber', 'employer'],
 ['abmarsch', 'march off'],
 ['gelände', 'terrain'],
 ['kasten', 'crate'],
 ['mittagessen', 'lunch'],
 ['kriegsgeschichte', 'war history'],
 ['zählen', 'to count'],
 ['schlachten', 'Butcher'],
 ['krieges', 'war'],
 ['überdauert', 'endures'],
 ['montiert', 'assembled'],
 ['neuigkeit', 'news'],
 ['schieß', 'shoot'],
 ['planspiel', 'simulation game'],
 ['angesetzt', 'scheduled'],
 ['disziplin', 'discipline'],
 ['frack', 'frack'],
 ['vergewaltigung', 'rape'],
 ['bekanntgabe', 'announcement'],
 ['anfangs', 'at first'],
 ['großfeuer', 'big fire'],
 ['nordflügel', 'north wing'],
 ['bereich', 'area'],
 ['inspektion', 'inspection'],
 ['zählen', 'to count'],
 ['ursache', 'cause'],
 ['unbekannt', 'unknown'],
 ['uhrzeit', 'time'],
 ['leitung', 'line'],
 ['qualms', 'smoke'],
 ['machthaber', 'ruler'],
 ['dankten', 'thanked'],
 ['nordflügel', 'north wing'],
 ['bereich', 'area'],
 ['ratsherren', 'councillors'],
 ['erlerntem', 'learned'],
 ['inspektion', 'inspection'],
 ['benachrichtige', 'notify'],
 ['maßgebend', 'authoritative'],
 ['theoretische', 'theoretical'],
 ['praktischen', 'practical'],
 ['begründen', 'justify'],
 ['entscheidenden', 'decisive'],
 ['nachteil', 'disadvantage'],
 ['bewegungsfreiheit', 'freedom of movement'],
 ['einschränkt', 'restricted'],
 ['einsatz', 'mission'],
 ['hydranten', 'hydrants'],
 ['nordflügel', 'north wing'],
 ['gebäudes', 'building'],
 ['branchen', 'Industry sectors'],
 ['wesentlich', 'significant'],
 ['starre', 'stare'],
 ['befolgen', 'follow'],
 ['nordflügel', 'north wing'],
 ['abbrennen', 'burn down'],
 ['auszusetzen', 'to suspend'],
 ['feuerlöscher', 'fire extinguisher'],
 ['arkaden', 'arcades'],
 ['angezeigt', 'displayed'],
 ['wirklichkeit', 'reality'],
 ['vorhanden', 'available'],
 ['ausrüstung', 'equipment'],
 ['lokals', 'local'],
 ['spaten', 'spade'],
 ['äxten', 'axes'],
 ['wasserschläuchen', 'water hoses'],
 ['mangelhaft', 'inadequate'],
 ['mangel', 'shortage'],
 ['offiziell', 'officially'],
 ['versäumt', 'missed'],
 ['entsprechende', 'appropriate'],
 ['erstatten', 'refund'],
 ['fehlerhaft', 'faulty'],
 ['verschoben', 'delay'],
 ['mitteilung', 'notice'],
 ['engländer', 'English people'],
 ['nordfranzösischen', 'northern French'],
 ['küste', 'coast'],
 ['kommts', "it's coming"],
 ['wörtlich', 'literally'],
 ['schlimmste', 'worst'],

 ['hauptsache', 'main thing'],
 ['überlegen', 'superior'],
 ['beisammen', 'together'],
 ['gleiten', 'slide'],
 ['schmieden', 'forge'],
 ['rechne', 'calculate'],
 ['händler', 'dealer'],
 ['ums', 'ums'],
 ['videotheken', 'video stores'],
 ['heinz', 'heinz'],
 ['vorliebe', 'preference'],
 ['pädagogische', 'educational'],
 ['produzieren', 'to produce'],
 ['prozent', 'percent'],
 ['idiotisch', 'idiotic'],
 ['risse', 'cracks'],
 ['feindliche', 'hostile'],
 ['aufgerieben', 'rubbed up'],
 ['gegnerischen', 'opposing ones'],
 ['brückenköpfe', 'bridgeheads'],
 ['zerschlagen', 'smashed'],
 ['feindlich', 'hostile'],
 ['landung', 'landing'],
 ['absichten', 'intentions'],
 ['verdeutlichen', 'clarify'],
 ['sätze', 'sentences'],
 ['seitz', 'since then'],
 ['stärkeren', 'stronger'],
 ['brückenköpfe', 'bridgeheads'],
 ['trübes', 'cloudy'],
 ['verstandes', 'understanding'],
 ['schaffe', 'create'],
 ['stratege', 'strategist'],
 ['unklar', 'not clear'],
 ['trost', 'comfort'],
 ['oberkommando', 'high command'],
 ['klarheit', 'clarity'],
 ['vorwerfen', 'accuse'],
 ['klipp', 'clip'],
 ['brückenköpfe', 'bridgeheads'],
 ['zerschlagen', 'smashed'],
 ['vierten', 'fourth'],
 ['ebenfalls', 'also'],
 ['klipp', 'clip'],
 ['landeinwärts', 'inland'],
 ['unklar', 'not clear'],
 ['absätze', 'heels'],
 ['widersprechen', 'contradict'],
 ['widerspruch', 'contradiction'],
 ['dienstherr', 'employer'],
 ['irreführung', 'misleading'],
 ['berichte', 'reports'],
 ['abwehr', 'defense'],
 ['funken', 'spark'],
 ['luftaufklärung', 'aerial reconnaissance'],
 ['belieben', 'at will'],
 ['dreißigjährigen', 'thirty year old'],
 ['schwachkopf', 'Moron'],
 ['widerspruch', 'contradiction'],
 ['schleudern', 'sling'],
 ['geistesblitzen', 'flashes of inspiration'],
 ['hammel', 'mutton'],
 ['herde', 'herd'],
 ['bayer', 'Bavarian'],
 ['ungern', 'reluctantly'],
 ['genie', 'genius'],
 ['diskutieren', 'discuss'],
 ['defizit', 'deficit'],
 ['käfer', 'Beetle'],
 ['streams', 'streams'],
 ['gesunden', 'healthy'],
 ['impotent', 'impotent'],
 ['menschenverstand', 'common sense'],
 ['brotbacken', 'baking bread'],
 ['fahrzeug', 'vehicle'],
 ['intellektuell', 'intellectually'],
 ['care', 'care'],
 ['besprechung', 'meeting'],
 ['eigenschaft', 'characteristic'],
 ['aufsichtsräte', 'supervisory boards'],
 ['betroffenen', 'affected'],
 ['ergebnis', 'result'],
 ['mitteilen', 'communicate'],
 ['mitteilen', 'communicate'],
 ['einwandfrei', 'flawless'],
 ['festgestellt', 'noted'],
 ['entsprechend', 'accordingly'],
 ['berichten', 'to report'],
 ['sprengung', 'blasting'],
 ['spuren', 'sense'],
 ['detonation', 'detonation'],
 ['einwandfreier', 'more flawless'],
 ['mageres', 'lean'],
 ['ergebnis', 'result'],
 ['selbstmord', 'Suicide'],
 ['annehmen', 'assume'],
 ['selbstläufer', 'self-perpetuating'],
 ['unehrenhaften', 'dishonorable'],
 ['entziehen', 'revoke'],
 ['zukommen', 'get'],
 ['meldungen', 'reports'],
 ['banken', 'banks'],
 ['wiederholt', 'repeated'],
 ['äußerungen', 'statements'],
 ['tatbestand', 'fact'],
 ['wehrkraftzersetzung', 'decomposition of military strength'],
 ['erfüllen', 'fulfill'],
 ['markus', 'Mark'],
 ['verteidigen', 'defend'],
 ['unvernünftige', 'unreasonable'],
 ['äußerungen', 'statements'],
 ['verbrecherische', 'criminal'],
 ['äußerungen', 'statements'],
 ['selbstmord', 'Suicide'],
 ['theorie', 'theory'],
 ['verfolgt', 'tracked'],
 ['ergebnis', 'result'],
 ['lösung', 'solution'],
 ['beteiligten', 'involved'],
 ['betracht', 'considered'],
 ['gezogen', 'pulled'],
 ['geringste', 'least'],
 ['anzeichen', 'signs'],
 ['annahme', 'assumption'],
 ['kenntnis', 'knowledge'],
 ['abgeschlossen', 'completed'],
 ['gegenwart', 'present'],
 ['verräterische', 'treacherous'],
 ['äußerungen', 'statements'],
 ['jüngeren', 'younger'],
 ['stimmten', 'agreed'],
 ['drauflos', 'go for it'],
 ['denunziert', 'denounced'],
 ['taktisch', 'tactical'],
 ['verlogen', 'lying'],
 ['schleim', 'mucus'],
 ['scheißer', 'shit'],
 ['herausgefunden', 'found out'],
 ['annehme', 'accept'],
 ['akten', 'files'],
 ['aufmachen', 'open up'],
 ['raten', 'guess'],
 ['barcoo', 'barcoo'],
 ['ergründen', 'fathom'],
 ['unfalls', 'accident'],
 ['anpacken', 'tackle'],
 ['überfällt', 'ambushed'],
 ['sachverständige', 'expert'],
 ['methodisch', 'methodical'],
 ['vorgehen', 'proceed'],
 ['hochbau', 'building construction'],
 ['oberland', 'Oberland'],
 ['ideen', 'ideas'],
 ['anregung', 'suggestion'],
 ['rechnet', 'calculates'],
 ['bedaure', 'sorry'],
 ['ober', 'upper'],
 ['schreibsachen', 'stationery'],
 ['detektive', 'detectives'],
 ['anhaltspunkten', 'clues'],
 ['indizien', 'circumstantial evidence'],
 ['widersprechen', 'contradict'],
 ['graben', 'dig'],
 ['group', 'group'],
 ['berichte', 'reports'],
 ['brei', 'porridge'],
 ['verweist', 'refers'],
 ['rauskam', 'came out'],
 ['knaben', 'boys'],
 ['widersprüchen', 'contradictions'],
 ['packen', 'pack'],
 ['befehls', 'command'],
 ['gemäss', 'according to'],
 ['spitze', 'great'],
 ['berichte', 'reports'],
 ['studiert', 'educated'],
 ['interessantes', 'interesting'],
 ['herausgekommen', 'came out'],
 ['lagen', 'lay'],
 ['hochbau', 'building construction'],
 ['zweck', 'purpose'],
 ['brennende', 'burning'],
 ['knalls', 'bang'],

 ['langsamen', 'slow'],
 ['brennenden', 'burning'],
 ['kombinieren', 'combine'],
 ['übrigen', 'remaining'],
 ['mager', 'skinny'],
 ['zitate', 'quotes'],
 ['hänseln', 'tease'],
 ['vorbeigehen', 'pass'],
 ['zitierte', 'quoted'],
 ['zitat', 'quote'],
 ['roms', 'roms'],
 ['pfeife', 'pipe'],
 ['getöse', 'noise'],
 ['dümpeln', 'bob'],
 ['vorbereitungen', 'preparations'],
 ['barockoper', 'baroque opera'],
 ['becker', 'Becker'],
 ['info', 'info'],
 ['sprengkörper', 'explosive devices'],
 ['schutzmauer', 'protective wall'],
 ['steillagen', 'steep slopes'],
 ['entdecken', 'discover'],
 ['entdeckung', 'discovery'],
 ['liefen', 'walked'],
 ['andreas', 'Andrew'],
 ['ammer', 'bunting'],
 ['schutzmauer', 'protective wall'],
 ['verstauchte', 'sprained'],
 ['vorfall', 'incident'],
 ['vorfall', 'incident'],
 ['erwähnt', 'mentioned'],
 ['erschien', 'appeared'],
 ['erwähnen', 'mention'],
 ['zwischenfall', 'incident'],
 ['loos', 'loos'],
 ['besah', 'looked at'],
 ['verstauchten', 'sprained'],
 ['schickte', 'sent'],
 ['sanitäter', 'paramedic'],
 ['umschlag', 'envelope'],
 ['vorfall', 'incident'],
 ['berichtet', 'reported'],
 ['andreas', 'Andrew'],
 ['ausführung', 'execution'],
 ['lakonischer', 'more laconic'],
 ['kürze', 'brevity'],
 ['unterstand', 'sheltered'],
 ['sanitäters', 'paramedic'],
 ['berichtet', 'reported'],
 ['verstauchungen', 'sprains'],
 ['verstärkung', 'reinforcement'],
 ['todes', 'death'],
 ['ausgangs', 'starting'],
 ['kameradschaftsabend', 'companionship evening'],
 ['stattfindet', 'takes place'],
 ['euere', 'yours'],
 ['mildere', 'milder'],
 ['beurteilung', 'assessment'],
 ['pfundskerl', 'pound guy'],
 ['auslaufen', 'leak'],
 ['geradezu', 'downright'],
 ['pleite', 'broke'],
 ['zugegeben', 'admitted'],
 ['schlau', 'clever'],
 ['kriterien', 'criteria'],
 ['verlesen', 'misread'],
 ['sonder', 'special'],
 ['drücken', 'press'],
 ['aufstieg', 'rise'],
 ['vauban', 'vauban'],
 ['befehlen', 'command'],
 ['fertigen', 'manufacture'],
 ['hinzuweisen', 'to point out'],
 ['abzuändern', 'to change'],
 ['glücklicherweise', 'fortunately'],
 ['empfehlen', 'recommend'],
 ['starte', 'start'],
 ['munition', 'ammunition'],
 ['bordeaux', 'bordeaux'],
 ['arbeitest', 'work'],
 ['naturen', 'natures'],
 ['lokus', 'locus'],
 ['zurücknehmen', 'take back'],
 ['bistum', 'diocese'],
 ['tüten', 'bags'],
 ['kneipen', 'pubs'],
 ['handeln', 'act'],
 ['erpresst', 'blackmailed'],
 ['angedroht', 'threatened'],
 ['boykottieren', 'boycott'],
 ['landpartie', 'country trip'],
 ['geplatzt', 'burst'],
 ['befohlen', 'ordered'],
 ['wichtiges', 'important'],
 ['einsam', 'lonely'],
 ['landpartie', 'country trip'],
 ['übungsplatz', 'practice area'],
 ['fähnchen', 'flag'],
 ['dienst', 'service'],
 ['rausgehen', 'go out'],
 ['gestellt', 'placed'],
 ['damenbesuch', 'ladies visit'],
 ['vorzufinden', 'to be found'],
 ['vollen', 'full'],
 ['rademacher', 'Rademacher'],
 ['meinerseits', 'on my part'],
 ['störe', 'disturb'],
 ['dienstes', 'service'],
 ['gestellt', 'placed'],
 ['verlobten', 'engaged'],
 ['entführen', 'kidnap'],
 ['sicht', 'view'],
 ['gelingt', 'succeed'],
 ['autorität', 'authority'],
 ['kameradschaftsabend', 'companionship evening'],
 ['anwesend', 'present'],
 ['köpfe', 'heads'],
 ['lücke', 'gap'],
 ['einschlugen', 'struck'],
 ['strafkicks', 'penalty kicks'],
 ['erzieherin', 'educator'],
 ['straff', 'tight'],
 ['abgetan', 'dismissed'],
 ['fry', 'fry'],
 ['wünscht', 'wishes'],
 ['dienstzimmer', 'duty room'],
 ['durchweg', 'consistently'],
 ['landpartie', 'country trip'],
 ['unterredung', 'conversation'],
 ['höchstens', 'at most'],
 ['höchstens', 'at most'],
 ['hübsches', 'pretty'],
 ['pfanne', 'pan'],
 ['verlobung', 'engagement'],
 ['blieb', 'stayed'],
 ['prügeln', 'beat'],
 ['lokal', 'local'],
 ['rüde', 'rude'],
 ['holzfäller', 'lumberjack'],
 ['manipulieren', 'manipulate'],
 ['erstens', 'first'],
 ['zurückgezogen', 'withdrawn'],
 ['schriftliche', 'written'],
 ['zweitens', 'Secondly'],
 ['berechtigter', 'more entitled'],
 ['verteidigung', 'defense'],
 ['wodurch', 'through which'],
 ['kneipe', 'bar'],
 ['hinrich', 'execute'],
 ['aegon', 'aegon'],
 ['neuerdings', 'recently'],
 ['blühenden', 'blooming'],
 ['blödsinn', 'nonsense'],
 ['ausgedacht', 'imagined'],
 ['absuchen', 'search'],
 ['anscheinend', 'apparently'],
 ['wüssten', 'would know'],
 ['nahmen', 'took'],
 ['gewandt', 'skillful'],
 ['himmels', 'heaven'],
 ['aussichten', 'outlook'],
 ['benommen', 'dazed'],
 ['gutem', 'good'],
 ['gehandelt', 'traded'],
 ['tolles', 'great'],
 ['fischer', 'fisherman'],
 ['fordern', 'demand'],
 ['inspektionen', 'inspections'],
 ['anschlag', 'attack'],
 ['brett', 'board'],
 ['berichtigung', 'correction'],
 ['lehren', 'to teach'],
 ['aegon', 'aegon'],
 ['streichen', 'cancel'],
 ['einzusetzen', 'to use'],
 ['befinden', 'condition'],
 ['unglücksgelände', 'accident site'],
 ['praktisch', 'practical'],
 ['wiederholen', 'repeat'],
 ['künstler', 'Artist'],
 ['termine', 'Events'],
 ['selben', 'the same'],
 ['orientierung', 'orientation'],
 ['rät', 'advises'],
 ['netz', 'network'],
 ['medizin', 'medicine'],
 ['schnüre', 'cords'],
 ['fliegt', 'flies'],
 ['enden', 'end up'],
 ['isolierband', 'electrical tape'],
 ['kombination', 'combination'],
 ['knall', 'bang'],
 ['schnüre', 'cords'],
 ['zentimeter', 'centimeter'],
 ['knall', 'bang'],
 ['leitet', 'directs'],
 ['verzögerung', 'delay'],
 ['sprengkörper', 'explosive devices'],
 ['schlagartig', 'suddenly'],
 ['längen', 'lengths'],
 ['bykow', 'bykov'],
 ['schnüre', 'cords'],
 ['gewählt', 'chosen'],
 ['länge', 'length'],
 ['anzünden', 'light'],
 ['entdeckung', 'discovery'],
 ['eingeteilt', 'assigned'],

 ['auszulegen', 'to lay out'],
 ['tarnen', 'camouflage'],
 ['sprengkapsel', 'detonator'],
 ['selbe', 'same'],
 ['durchtriebener', 'more cunning'],
 ['ausbeißen', 'bite out'],
 ['abenden', 'evenings'],
 ['baku', 'baku'],
 ['sprung', 'Leap'],
 ['verstaucht', 'sprained'],
 ['verging', 'passed away'],
 ['zweifellos', 'undoubtedly'],
 ['leutnant', 'lieutenant'],
 ['erklärung', 'explanation'],
 ['bayern', 'Bavaria'],
 ['schnitt', 'cut'],
 ['stumme', 'silent ones'],
 ['herunter', 'down'],
 ['brenndauer', 'burning time'],
 ['besiegelt', 'sealed'],
 ['sprengen', 'bust'],
 ['schlottern', 'shiver'],
 ['sprengkapsel', 'detonator'],
 ['attrappe', 'dummy'],
 ['dummkopf', 'fool'],
 ['verkürzen', 'shorten'],
 ['absichtlich', 'intentionally'],
 ['herbeigeführt', 'brought about'],
 ['parkour', 'parkour'],
 ['vaterlandsverräter', 'traitor to the fatherland'],
 ['führers', 'leader'],
 ['neugierig', 'curious'],
 ['drohungen', 'threats'],
 ['interessieren', 'interested'],
 ['geständnis', 'confession'],
 ['classics', 'classics'],
 ['ankommen', 'arrive'],
 ['effizienten', 'efficient'],
 ['ernsthafte', 'serious'],
 ['beantworten', 'answer'],
 ['ordnen', 'arrange'],
 ['notizen', 'notes'],
 ['erklärungen', 'Explanations'],
 ['schlägereien', 'fights'],
 ['fähnrichs', 'ensign'],
 ['böhmke', 'böhmke'],
 ['leidtragende', 'suffering'],
 ['schweine', 'pigs'],
 ['schützenbrücke', 'protect bridge'],
 ['schlugen', 'hit'],
 ['geburtstagsfeier', 'birthday party'],
 ['mitgesungen', 'sang along'],
 ['prügelei', 'brawl'],
 ['kragen', 'collar'],
 ['rednitz', 'rednitz'],
 ['gerieten', 'got into trouble'],
 ['aneinander', 'to each other'],
 ['rückzugs', 'withdrawal'],
 ['armeen', 'armies'],
 ['gezeichnet', 'drawn'],
 ['russland', 'Russia'],
 ['dreien', 'three'],
 ['tyrannisieren', 'bullying'],
 ['radikale', 'radical'],
 ['nationalsozialisten', 'National Socialists'],
 ['bauch', 'belly'],
 ['zurückzuschlagen', 'to strike back'],
 ['erdlöchern', 'holes in the ground'],
 ['gesessen', 'sat'],
 ['überwinden', 'overcome'],
 ['verkürzt', 'shortened'],
 ['tieren', 'animals'],
 ['zitieren', 'quote'],
 ['gesunde', 'healthy'],
 ['gerechtigkeit', 'justice'],
 ['charakter', 'character'],
 ['angehört', 'heard'],
 ['power', 'power'],
 ['verkürzt', 'shortened'],
 ['aussage', 'expression'],
 ['eid', 'oath'],
 ['persönliche', 'personal'],
 ['geständnis', 'confession'],
 ['gerichtsverfahren', 'legal proceedings'],
 ['eröffnet', 'opened'],
 ['geständnis', 'confession'],
 ['hochbaus', 'building construction'],
 ['kommission', 'Commission'],
 ['ausschlussverfahren', 'exclusion process'],
 ['notwendige', 'necessary'],
 ['rednet', 'talks'],
 ['fraktion', 'fraction'],
 ['kriegslist', 'stratagem'],
 ['erdloch', 'hole in the ground'],
 ['philosophen', 'philosophers'],
 ['nietzsche', 'Nietzsche'],
 ['schießübung', 'shooting practice'],
 ['tagebuch', 'diary'],
 ['lieder', 'Songs'],
 ['spanisches', 'Spanish'],
 ['führers', 'leader'],
 ['staub', 'dust'],
 ['asche', 'ash'],
 ['schreiber', 'writer'],
 ['barcode', 'barcode'],
 ['verehrt', 'adored'],
 ['einiges', 'some'],
 ['verfahren', 'procedure'],
 ['zuhörer', 'listeners'],
 ['knifflige', 'tricky ones'],
 ['verfahrenstechnischen', 'procedural engineering'],
 ['auftauchen', 'turn up'],
 ['eingreifen', 'intervention'],
 ['stuhl', 'chair'],
 ['ausschlussverfahren', 'exclusion process'],
 ['dahinter', 'behind it'],
 ['vertreter', 'representative'],
 ['verfahrens', 'procedural'],
 ['begründung', 'Reason'],
 ['verfügung', 'disposal'],
 ['kommission', 'Commission'],
 ['beschuldigtem', 'accused'],
 ['begründung', 'Reason'],
 ['mündlich', 'oral'],
 ['vortragen', 'present'],
 ['nationalsozialist', 'National Socialist'],
 ['zügelloser', 'more licentious'],
 ['fanatiker', 'fanatic'],
 ['extremen', 'extremes'],
 ['terrorregime', 'terror regime'],
 ['henrich', 'Henry'],
 ['empfehlung', 'recommendation'],
 ['partei', 'Political party'],
 ['zeigte', 'showed'],
 ['ehrgeizig', 'ambitious'],
 ['rücksichtsloser', 'more ruthless'],
 ['streber', 'nerd'],
 ['komplex', 'complex'],
 ['beherrscht', 'controlled'],
 ['förderer', 'sponsor'],
 ['enttäuschen', 'disappoint'],
 ['gewaltakt', 'act of violence'],
 ['gewissheit', 'certainty'],
 ['offiziers', 'officer'],
 ['examen', 'exam'],
 ['kurze', 'short'],
 ['negativen', 'negative'],
 ['hinterlassenen', 'left behind'],
 ['ungeeignet', 'not suitable'],
 ['qualifiziert', 'qualified'],
 ['dementsprechend', 'accordingly'],
 ['verwarnt', 'warned'],
 ['dadurch', 'through this'],
 ['beseitigt', 'eliminated'],
 ['beschuldigen', 'accuse'],
 ['verkürzt', 'shortened'],
 ['geständnis', 'confession'],
 ['fenrich', 'fenrich'],
 ['solches', 'such'],
 ['geständnis', 'confession'],
 ['abgelegt', 'filed'],
 ['unsinnige', 'nonsensical'],
 ['verwahrt', 'kept'],
 ['kabel', 'cable'],
 ['geglaubt', 'believed'],
 ['kritischen', 'critical'],
 ['zinskurve', 'yield curve'],
 ['lügt', 'lies'],
 ['gerühmt', 'boasted'],
 ['gedroht', 'threatened'],
 ['schwerwiegende', 'serious'],
 ['anschuldigungen', 'accusations'],
 ['henrich', 'Henry'],
 ['lübke', 'luebke'],
 ['einfachen', 'simple'],
 ['aufzurufen', 'to call'],
 ['vernehmung', 'interrogation'],
 ['zeuge', 'witness'],
 ['vorgeladen', 'preloaded'],
 ['schildern', 'describe'],
 ['beobachtungen', 'observations'],
 ['kritischen', 'critical'],
 ['unglücks', 'misfortune'],
 ['nachmittags', 'afternoon'],
 ['wahrheitsgemäß', 'truthfully'],
 ['aussage', 'expression'],
 ['erdloch', 'hole in the ground'],
 ['verletzten', 'injured'],
 ['herüber', 'over'],
 ['setzte', 'sat'],
 ['sprachen', 'languages'],
 ['zurückging', 'went back'],
 ['durfte', 'could'],
 ['explosion', 'explosion'],
 ['inkl', 'Incl'],
 ['vorgestern', 'the day before yesterday'],
 ['anwesenheit', 'presence'],
 ['trainer', 'Trainer'],
 ['pionier', 'pioneer'],
 ['hantieren', 'handle'],
 ['stattgefunden', 'took place'],
 ['inkl', 'Incl'],
 ['erinnerung', 'memory'],
 ['bedroht', 'threatened'],
 ['gezwungen', 'forced'],
 ['aussage', 'expression'],
 ['beschuldigt', 'accused'],
 ['verdächtig', 'suspicious'],
 ['greift', 'takes hold'],
 ['kritischen', 'critical'],
 ['vorsitzende', 'chairman'],
 ['selbstverständlich', 'of course'],
 ['verwahre', 'keep safe'],
 ['unzulässig', 'inadmissible'],
 ['zeuge', 'witness'],
 ['brest', 'brest'],
 ['drohte', 'threatened'],
 ['kalken', 'lime'],
 ['verknüpft', 'connected'],
 ['freien', 'free'],
 ['kriegsgericht', 'court-martial'],
 ['durcheinander', 'mixed up'],
 ['revier', 'territory'],
 ['stube', 'room'],
 ['angelegenheit', 'matter'],
 ['hass', 'hate'],
 ['komplizen', 'accomplices'],
 ['hofbauers', 'hofbauers'],
 ['diffus', 'diffuse'],
 ['verstauchung', 'sprain'],
 ['finte', 'feint'],
 ['anbau', 'cultivation'],
 ['chor', 'Choir'],
 ['wegzulocken', 'to lure away'],
 ['beantrage', 'apply'],
 ['verfahrens', 'procedural'],
 ['beihilfe', 'aid'],
 ['mitwirkung', 'participation'],
 ['fielen', 'fell'],
 ['bewiesen', 'proven'],
 ['beeinflussen', 'influence'],
 ['wehren', 'defend'],
 ['unbarmherzige', 'ruthless'],
 ['einkesselung', 'encirclement'],
 ['zeuge', 'witness'],
 ['bewiesen', 'proven'],
 ['billige', 'cheap'],
 ['unbeeinflusst', 'unaffected'],
 ['intrige', 'intrigue'],
 ['begeisterter', 'more enthusiastic'],
 ['führers', 'leader'],
 ['unverschämtheiten', 'impudence'],
 ['herrlich', 'splendid'],
 ['gegenteil', 'opposite'],
 ['geräumt', 'cleared'],
 ['bockshorn', 'fenugreek'],
 ['buchbar', 'bookable'],
 ['angehörigen', 'relatives'],
 ['überführt', 'transferred'],
 ['würdige', 'worthy'],
 ['feier', 'celebration'],
 ['veranstalten', 'organize'],
 ['rehabilitieren', 'rehabilitate'],
 ['totenfeier', 'funeral celebration'],
 ['selbstmörder', 'suicidal'],
 ['bescheinigen', 'certify'],
 ['verstauen', 'stow away'],
 ['draufstehen', 'stand on it'],
 ['reinkriegt', 'get in'],
 ['schwindel', 'dizziness'],
 ['befreundet', 'friends'],
 ['gezwungen', 'forced'],
 ['toren', 'goals'],
 ['eingestanden', 'admitted'],
 ['stürmer', 'striker'],
 ['linie', 'line'],
 ['angetan', 'impressed'],
 ['pille', 'pill'],
 ['ruhigen', 'calm'],
 ['verbringen', 'spend'],
 ['stich', 'Sting'],
 ['heutzutage', 'nowadays'],
 ['mühe', 'effort'],
 ['fabelhaft', 'fabulous'],
 ['gratuliere', 'congratulate'],
 ['kompliziert', 'complicated'],
 ['frecher', 'cheekier'],
 ['unvorsichtig', 'careless'],
 ['ziehe', 'pull'],
 ['riesenfisch', 'giant fish'],
 ['erlaubnis', 'permission'],
 ['stetiger', 'more steady'],
 ['zeuge', 'witness'],
 ['vielversprechenden', 'promising'],
 ['offiziersanwärter', 'officer candidate'],
 ['getrieben', 'driven'],
 ['verdrehen', 'twist'],
 ['tatsachen', 'facts'],
 ['zurückversetzt', 'set back'],
 ['würdige', 'worthy'],
 ['verbiete', 'forbid'],
 ['dienstältester', 'senior'],
 ['vorläufiger', 'more preliminary'],
 ['kommandeur', 'commander'],
 ['verhafteten', 'arrested'],
 ['führers', 'leader'],
 ['notwendige', 'necessary'],
 ['militärische', 'military'],
 ['maßnahmen', 'measures'],
 ['politischen', 'political'],
 ['führung', 'guide'],
 ['umsturz', 'overthrow'],
 ['gewaltsamer', 'more violent'],
 ['beseitigung', 'elimination'],
 ['schreibtisch', 'desk'],
 ['freibier', 'free beer'],
 ['schafott', 'scaffold'],
 ['staates', 'state'],
 ['langem', 'long'],
 ['beobachtung', 'observation'],
 ['schönheitsfehler', 'beauty mistake'],

 ['gerichtet', 'directed'],
 ['treibende', 'driving'],
 ['geplanten', 'planned'],
 ['beheben', 'remedy'],
 ['anzubieten', 'to offer'],
 ['garantieren', 'to guarantee'],
 ['verschwörer', 'conspirators'],
 ['beamten', 'officials'],
 ['haft', 'liable'],
 ['kochen', 'cook'],
 ['pasta', 'pasta'],
 ['privatangelegenheiten', 'private matters'],
 ['nachdrücklich', 'emphatically'],
 ['majoran', 'marjoram'],
 ['festessen', 'feast'],
 ['unverbesserlicher', 'more incorrigible'],
 ['trauerfeier', 'funeral service'],
 ['traurige', 'sad ones'],
 ['gedenken', 'commemorate'],
 ['fanatismus', 'bigotry'],
 ['hervorragender', 'more outstanding'],
 ['schüler', 'pupils'],
 ['glanzvollen', 'brilliant'],
 ['karriere', 'Career'],
 ['freiwilliger', 'more voluntary'],
 ['schlug', 'hit'],
 ['ergriffenheit', 'emotion'],
 ['tragödie', 'tragedy'],
 ['erhebendes', 'uplifting'],
 ['versorgt', 'provided'],
 ['logischen', 'logical'],
 ['malers', 'painter'],
 ['zurückversetzen', 'put back'],
 ['bankrott', 'bankrupt'],
 ['ersticken', 'suffocate'],
 ['freibrief', 'carte blanche'],
 ['unfehlbarkeit', 'infallibility'],
 ['friede', 'peace'],
 ['versetzt', 'offset'],
 ['schreibe', 'write'],
 ['marschbefehl', 'marching orders'],
 ['gekonnt', 'skillfully'],
 ['gestapo', 'Gestapo'],
 ['ausrichten', 'align'],
 ['machtlos', 'powerless'],
 ['tomaten', 'tomatoes'],
 ['gehängt', 'hanged'],
 ['gebettelt', 'begged'],
 ['vernünftig', 'reasonable'],
 ['sofortiger', 'more immediate'],
 ['auffassung', 'opinion'],
 ['empfehle', 'recommend'],
 ['parieren', 'parry'],
 ['kleinholz', 'kindling'],
 ['segen', 'blessing'],
 ['schriftlich', 'in written form'],
 ['level', 'level'],
 ['einführung', 'introduction'],
 ['amt', 'government office'],
 ['fresse', 'eat'],
 ['belegt', 'proven'],
 ['fällen', 'fall'],
 ['rechnen', 'calculate'],
 ['sofortiger', 'more immediate'],
 ['oberländerin', 'Oberlander'],
 ['vakuum', 'vacuum'],
 ['eingestellt', 'set'],
 ['ereignisreich', 'eventful'],
 ['taktischen', 'tactical'],
 ['feldmark', 'Feldmark'],
 ['mäßig', 'moderate'],
 ['unteren', 'lower'],
 ['antreten', 'compete'],
 ['verhaftung', 'arrest'],
 ['beckers', 'Beckers'],
 ['begab', 'gifted'],
 ['blicke', 'looks'],
 ['nachtübung', 'night exercise'],
 ['teile', 'share'],
 ['zerbricht', 'breaks'],
 ['höher', 'higher'],
 ['nachdenklich', 'pensive'],
 ['roten', 'redden'],
 ['frachters', 'freighter'],
 ['rücksprache', 'consultation'],
 ['lehrkörpers', 'teaching staff'],
 ['beschlossen', 'decided'],
 ['nachtübung', 'night exercise'],
 ['räder', 'wheels'],
 ['übernimmt', 'takes over'],


 ['vorbildlicher', 'more exemplary'],
 ['Gefangener', 'Prisoner'],
 ['ordentlich', 'orderly'],
 ['arbeitswillig', 'willing to work'],
 ['höflich', 'polite'],
 ['fleißig', 'diligent'],
 ['abgesessen', 'served'],
 ['ranzukommen', 'to get there'],
 ['quitt', 'quit'],
 ['scheinwerferlich', 'spotlight'],
 ['niesen', 'sneeze'],
 ['Tonbandgerät', 'Tape recorder'],
 ['gehofft', 'hoped'],
 ['klüger', 'smarter'],
 ['schlauer', 'smarter'],
 ['älter', 'older'],
 ['exempling', 'exemplary'],
 ['wandelnde', 'walking'],
 ['Zielscheibe', 'Target'],
 ['abholen', 'pick up'],
 ['reingehen', 'go in'],
 ["mach's", 'do it'],
 ['crimer', 'crimer'],
 ['record', 'record'],
 ['rauskommt', 'comes out'],
 ['Einsamkeit', 'lonliness'],
 ['Einsamkeit', 'lonliness'],
 ['Cremer', 'Cremer'],
 ['genießen', 'enjoy'],
 ['Polizeipräsidium', 'Police headquarters'],
 ['Raubdezernat', 'Robbery squad'],
 ['Bulle', 'Bull'],
 ['verklage', 'sue'],
 ['leumundungen', 'reputations'],
 ['weitergebildet', 'further trained'],
 ['Bürger', 'Citizens'],
 ['Leichenhalle', 'Mortuary'],
 ['angesehen', 'viewed'],
 ['schätz', 'guess'],
 ['aufnehmen', 'to record'],
 ['angreifen', 'attack'],
 ['Versteck', 'Hideout'],
 ['geträumt', 'dreamed'],
 ['irgendwohin', 'somewhere'],
 ['geträumt', 'dreamed'],
 ['Susi', 'Suzy'],
 ['verflucht', 'cursed'],
 ['langen', 'long'],
 ['verändern', 'change'],
 ['überschrieben', 'overwritten'],
 ['geschenkt', 'gifted'],
 ['Begrüßung', 'greeting'],
 ['quetscht', 'squeezes'],
 ['daheim', 'home'],
 ['Toast', 'toast'],
 ['Trinkspruch', 'Toast'],
 ['menschliche', 'human'],
 ['Nici', 'Nici'],
 ['einigermaßen', 'to some extent'],
 ['Wutanfälle', 'Tantrums'],
 ['selten', 'rarely'],
 ['nix', 'nothing'],
 ['ergiebig', 'productive'],
 ["sieht's", 'see it'],
 ['Beute', 'prey'],
 ['richtiges', 'correct'],
 ['Steak', 'steak'],
 ['dickes', 'thick'],
 ['schneiden', 'cut'],
 ['ausspannen', 'relax'],
 ['betrifft', 'regards'],
 ['Angabe', 'specification'],
 ['geträumt', 'dreamed'],
 ['fange', 'catch'],
 ['besprechen', 'discuss'],
 ['lade', 'load'],
 ['gewaltigste', 'most powerful'],
 ['Steak', 'steak'],
 ['Strickzeug', 'Knitting'],
 ['weiterleiten', 'forward'],
 ['erstmal', 'for now'],
 ['Einkaufen', 'Shopping'],
 ['womit', 'by which'],
 ['Postsäcke', 'Mailbags'],
 ['genäht', 'sewn'],
 ['Kartons', 'Cardboard boxes'],
 ['geklebt', 'glued'],
 ['Jute', 'jute'],
 ['gekämmt', 'combed'],
 ['gespart', 'saved'],
 ['steinreich', 'filthy rich'],
 ['brat', 'fry'],
 ['Fass', 'barrel'],
 ['betrifft', 'regards'],
 ['immerhin', 'anyway'],
 ['geschüttet', 'poured'],
 ['zuschlägt', 'strikes'],
 ['immerhin', 'anyway'],
 ['widerlich', 'disgusting'],
 ['lausige', 'lousy'],
 ['Samt', 'velvet'],
 ['Seide', 'silk'],
 ['übers', 'trans'],
 ['Shows', 'shows'],
 ['geplant', 'planned'],
 ['Anklage', 'accusation'],
 ['Hoffenheim', 'Hoffenheim'],
 ['Staatsanwalt', 'Prosecutor'],
 ['auszusagen', 'to testify'],
 ['Staatsanwalt', 'Prosecutor'],
 ['schnappte', 'snapped'],
 ['Fersen', 'heels'],
 ['Maulkorb', 'muzzle'],
 ['angelegt', 'created'],
 ['Kabeln', 'Cable'],
 ['Geschäftsmann', 'Businessman'],
 ['immerhin', 'anyway'],
 ['immerhin', 'anyway'],
 ['Lohngelder', 'Wage money'],
 ['viertel', 'quarter'],
 ['vorgeschlagen', 'suggested'],
 ['abgewickelt', 'handled'],
 ['wendet', 'applies'],
 ['schweren', 'heavy'],
 ['nützlicher', 'more useful'],
 ['Gemeinde', 'Community'],
 ['vergisst', 'forgets'],
 ['vergesse', 'forget'],
 ['bezahlst', 'pay'],
 ['angespannt', 'tense'],
 ['verlasse', 'leave'],
 ['absolute', 'absolute'],
 ['Loyalität', 'loyalty'],
 ['Brands', 'Brands'],
 ['Trott', 'Trot'],
 ['Gewissensbisse', 'remorse'],
 ['hintergehen', 'deceive'],
 ['aufgewecktes', 'bright'],
 ['Bildschirm', 'Screen'],
 ['regelmäßig', 'regularly'],
 ['klammern', 'cling'],
 ['Fotografen', 'photographers'],
 ['Story', 'story'],
 ['Eisen', 'iron'],
 ["gibt's", 'there is'],
 ['Bradley', 'Bradley'],
 ['zuschlagen', 'strike'],
 ['richtiges', 'correct'],
 ['echter', 'real'],
 ['woran', 'what'],
 ['schmecke', 'taste'],
 ['Zwiebeln', 'Onions'],
 ['lösen', 'solve'],
 ['vergrabenen', 'buried'],
 ['Handy', 'mobile'],
 ['resortsvater', 'resort father'],
 ['Italiener', 'Italian'],
 ['Samir', 'Samir'],
 ['verpfiffen', 'ratted out'],
 ['Rotwein', 'red wine'],
 ['vorlaufen', 'run forward'],
 ['Lungenentzündung', 'lung infection'],
 ['geholt', 'fetched'],
 ['überredet', 'persuaded'],
 ['Viertelmillion', 'Quarter million'],
 ['überstanden', 'survived'],
 ['einzig', 'only'],
 ['verstrickt', 'entangled'],
 ['hineingezogen', 'pulled in'],
 ['Sams', 'Sams'],
 ['weggezogen', 'pulled away'],
 ['braunen', 'tan'],
 ['mochtest', 'liked'],
 ['farmers', 'farmers'],
 ['Anteil', 'Portion'],
 ['mickrigen', 'puny'],
 ['Tausender', 'thousands'],
 ['gespart', 'saved'],
 ['Viertelmillion', 'Quarter million'],
 ['viertel', 'quarter'],
 ['sauberes', 'clean'],
 ['beschaffen', 'procure'],
 ['hingehen', 'go there'],
 ['aufheben', 'lift'],
 ['Helfer', 'helper'],
 ['riechen', 'smell'],
 ['Scheine', 'Bills'],
 ['niese', 'sneeze'],
 ['Taschentuch', 'Handkerchief'],
 ['rechten', 'right'],
 ['geborenekowski', 'Bornekowski'],
 ['Verhandlungen', 'negotiations'],
 ['Jugendgericht', 'Juvenile Court'],
 ['Beteiligung', 'participation'],
 ['bestellt', 'ordered'],
 ['Frechheit', 'impudence'],
 ['bieten', 'offer'],
 ['Einbruch', 'Burglary'],
 ['Verhaftungen', 'Arrests'],
 ['knallharter', 'tougher'],
 ['Bursche', 'fellow'],
 ['hingehen', 'go there'],
 ['Fersen', 'heels'],
 ['überwachen', 'monitor'],
 ['plattfüßige', 'flat-footed'],
 ['herumschleicht', 'sneaking around'],
 ['ändert', 'changes'],
 ['albernes', 'silly'],
 ['aufspüren', 'track down'],
 ['Lohngelder', 'Wage money'],
 ['Viertelmillion', 'Quarter million'],
 ['spielst', 'play'],
 ['unschuldige', 'innocent ones'],
 ['ertragen', 'bear'],
 ['anfasst', 'touches'],
 ['betest', 'pray'],
 ['bete', 'pray'],
 ['Südamerika', 'South America'],
 ['überlegt', 'considered'],
 ['Schlachthöfen', 'slaughterhouses'],
 ['beschatten', 'shade'],
 ['Atemzug', 'breath'],
 ["soll's", 'it should'],
 ['nachspioniert', 'spied on'],
 ['rumschnüffeln', 'snoop around'],
 ['schnellsten', 'fastest'],
 ['erstmal', 'for now'],
 ['Durchgang', 'passage'],
 ['bekannter', 'more famous'],
 ['gewohnt', 'used'],
 ['vermietet', 'rented'],
 ['schmutzigen', 'dirty'],
 ['gewohnt', 'used'],
 ['undichtung', 'leakage'],
 ['geschenkt', 'gifted'],
 ['Pulli', 'Sweater'],
 ['Hosen', 'Pants'],
 ['Werkstatt', 'Workshop'],
 ['aufgehört', 'stopped'],
 ['misses', 'misses'],
 ['ruso', 'ruso'],
 ['einigermaßen', 'to some extent'],
 ['vergingen', 'passed away'],
 ['Werkstatt', 'Workshop'],
 ['weggezogen', 'pulled away'],
 ['schwaches', 'weak'],
 ['Gedächtnis', 'memory'],
 ['Kisten', 'boxes'],
 ['packte', 'grabbed'],
 ['grün', 'green'],
 ['Finanzierungsgesellschaft', 'financing company'],
 ['Sammy', 'Sammy'],
 ['Kommode', 'Dresser'],
 ['eingeklemmt', 'trapped'],
 ['Geschirr', 'Dishes'],
 ['klug', 'smart'],
 ['einmonatiges', 'one month'],
 ['Jubiläum', 'anniversary'],
 ['Diamantring', 'Diamond ring'],
 ['Faust', 'fist'],
 ['beunruhigen', 'worry'],
 ['lohngeldern', 'wage money'],
 ['aufhöre', 'stop'],
 ['frisieren', 'comb'],
 ['sicheren', 'safe'],
 ['Jobsuche', 'Job search'],
 ['eingestiegen', 'got in'],
 ['Autowerkstatt', 'Auto repair shop'],
 ['steinreich', 'filthy rich'],
 ['Schlauch', 'Hose'],
 ['geldsvater', 'money father'],
 ['verrieche', 'smell'],
 ['gestutzt', 'trimmed'],
 ['Kaff', 'Koff'],
 ['Gale', 'Gael'],
 ['Braten', 'roast meat'],
 ['doppelt', 'double'],
 ['eingepackt', 'packed'],
 ['Ventile', 'Valves'],
 ['einstellen', 'set'],
 ['Zigarette', 'Cigarette'],
 ['angenommen', 'assumed'],
 ['monatiges', 'monthly'],
 ['Jubiläum', 'anniversary'],
 ['Gelder', 'funds'],
 ['Vordermann', 'frontman'],
 ['poliert', 'polished'],
 ['schmiert', 'lubricates'],
 ['Sitzbezüge', 'Seat covers'],
 ['dazugehört', 'belongs to it'],
 ['vorgestellt', 'presented'],
 ['Jubiläumsgeschenk', 'Anniversary gift'],
 ['wunderschönes', 'beautiful'],
 ['aufmerksam', 'attentive'],

 ['setze', 'set'],
 ['Krankenwagen', 'ambulance'],
 ['kämmen', 'comb'],
 ['begleichen', 'settle'],
 ['angehindert', 'hindered'],
 ['fromm', 'religious'],
 ['abhauen', 'run away'],
 ['Verfassung', 'Constitution'],
 ['aufzuhalten', 'to stop'],
 ['Rücksitz', 'back seat'],
 ['betonte', 'emphasized'],
 ['festnehmen', 'arrest'],
 ['spaziert', 'walks'],
 ['zeichnen', 'draw'],
 ['Müdigkeit', 'fatigue'],
 ['überzeugen', 'convince'],
 ['nützt', 'useful'],
 ['schießt', 'shoots'],
 ['umlegen', 'fold over'],
 ['Schläger', 'bat'],
 ['heilsam', 'healing'],
 ['ruiniert', 'ruined'],
 ['frische', 'freshness'],
 ['hoffen', 'hope'],
 ['gerissene', 'cunning'],
 ['gerissen', 'cracked'],
 ['freundliches', 'friendly'],
 ['ausgebaute', 'expanded'],
 ['fehlen', 'miss'],
 ['Schläger', 'bat'],
 ['gefesselt', 'tied up'],
 ['verkehrt', 'wrong'],
 ['gemerkt', 'noticed'],
 ['bedauern', 'regret'],
 ['fängst', 'catch'],
 ['zurückgegeben', 'returned'],
 ['feststellen', 'notice'],
 ['ausrasten', 'freak out'],
 ['rangehe', 'answer'],
 ['Kehle', 'throat'],
 ['Gründen', 'Found'],
 ['Hoffman', 'Hoffman'],
 ['Sitz', 'Seat'],
 ['funktionieren', 'function'],
 ['Schalldämpfer', 'silencer'],
 ['prüfen', 'check'],
 ['Speisekarte', 'Menu'],
 ['überraschen', 'to surprise'],
 ['bestimmtes', 'certain'],
 ['genügt', 'enough'],
 ['rauszukriegen', 'to get out'],
 ['durchmachen', 'go through'],
 ['ausgestorben', 'extinct'],
 ['Friedhof', 'Cemetery'],
 ['betreiben', 'operate'],
 ['Tankstelle', 'Gas station'],
 ['herkommt', 'comes here'],
 ['ausgegraben', 'unearthed'],
 ['durchgegangen', 'passed through'],
 ['Hinweis', 'Notice'],
 ['Russo', 'Russo'],
 ['Neuigkeiten', 'news'],
 ['längs', 'along'],
 ['Rücksitz', 'back seat'],
 ['erwischen', 'catch'],
 ['lenkt', 'directs'],
 ['überlassen', 'left'],
 ['Lenk', 'Steering'],
 ['fortgehen', 'go away'],
 ['verleihen', 'to lend'],
 ['bedrängen', 'harass'],
 ['zurückgibst', 'give back'],
 ['zurückgeben', 'hand back'],
 ["versuch's", 'try it'],
 ['gestärkt', 'strengthened'],
 ['geehrt', 'honored'],
 ['wärst', 'would be'],
 ['geil', 'cool'],
 ['schuldest', 'you owe'],
 ['verschenke', 'give away'],
 ['gewaltiger', 'more powerful'],
 ['wertvollsten', 'most valuable'],
 ['Kostproben', 'Samples'],
 ['servieren', 'serve'],
 ['einzelne', 'separate'],
 ['angeschweißt', 'welded on'],
 ['verschwende', 'waste'],
 ['Schatzi', 'honey'],
 ['mieses', 'lousy'],
 ['irgendein', 'any'],
 ['Hefe', 'Yeast'],
 ['Russo', 'Russo'],
 ['anständigen', 'decent'],
 ['wohne', 'live'],
 ['schlafe', 'sleep'],
 ['gingst', 'went'],
 ['winziges', 'tiny'],
 ['Fingern', 'fingers'],
 ['heranzukommen', 'to approach'],
 ['juckt', 'itches'],
 ['raushalten', 'keep out'],
 ['gestohlenen', 'stolen'],
 ['hingehören', 'belong'],
 ['satt', 'Fed up'],
 ['rauskomme', 'come out'],
 ['bekehrt', 'converted'],
 ['Geister', 'Ghosts'],
 ['Auftritt', 'Appearance'],
 ['Rahmen', 'Frame'],
 ['verschenke', 'give away'],
 ['verbrennen', 'burn'],
 ['fantasierte', 'fantasized'],
 ['rannte', 'ran'],
 ['riefen', 'shouted'],
 ['angesprochen', 'addressed'],
 ['verdrängen', 'push away'],
 ['losgewandelt', 'changed loose'],
 ['gezittert', 'trembled'],
 ['geiles', 'awesome'],
 ['zweitürige', 'two-door'],
 ['weshalb', 'for what reason'],
 ['identifiziert', 'identified'],
 ['autome', 'autome'],
 ['Gangster', 'gangster'],
 ['Flucht', 'Escape'],
 ['erschießt', 'shot'],
 ['herkommst', 'you come from'],
 ['wärst', 'would be'],
 ['begraben', 'to bury'],
 ['einließ', 'let in'],
 ['wärst', 'would be'],
 ['empfindest', 'feel'],
 ['weshalb', 'for what reason'],
 ['Schinken', 'Ham'],
 ['wehe', 'woe'],
 ['zurecht', 'right'],
 ['Maschinengewehr', 'machine gun'],
 ['Lenk', 'Steering'],
 ['lenken', 'to steer'],
 ['lenken', 'to steer'],
 ['Fischen', 'fishing'],
 ['taubenmester', 'pigeon nester'],
 ['Lenk', 'Steering'],
 ['Auskommen', 'Getting by'],
 ['Schild', 'Sign'],
 ['Tankstelle', 'Gas station'],
 ['Straßen', 'streets'],
 ['prüfen', 'check'],
 ['ausfallen', 'stand out'],
 ['tanken', 'refuel'],
 ['Davy', 'Davy'],
 ['zurückzugehen', 'to go back'],
 ['zurückzugehen', 'to go back'],
 ['Autoschlüssel', 'car key'],
 ['ungerecht', 'unfair'],
 ['Hörmann', 'Hörmann'],
 ['bemüht', 'engaged'],
 ['zuallererst', 'first of all'],
 ['hergebracht', 'brought here'],
 ['abgemacht', 'agreed'],
 ['zusätzlich', 'additionally'],
 ['einsteigen', 'get in'],
 ['offnen', 'open'],
 ['blauen', 'blue'],
 ['wiederhole', 'repeat'],
 ['gesichtet', 'spotted'],
 ['Cabrio', 'Convertible'],
 ['Einheit', 'Unit'],
 ['nachsehen', 'check'],
 ['Hersteller', 'Manufacturer'],
 ['ausnehmen', 'exclude'],
 ['dünner', 'thinner'],
 ['dümmer', 'dumber'],
 ['Zimmermiete', 'Room rent'],
 ['abwickeln', 'transact'],
 ['reinzulegen', 'to put in'],
 ['geringsteen', 'least'],
 ['zurückziehen', 'withdraw'],
 ['Kloster', 'monastery'],
 ['eingesperrt', 'locked'],
 ['überziehen', 'cover'],
 ['Fleischer', 'butcher'],
 ['nikaden', 'nicadas'],
 ['irgendeine', 'any'],
 ['durchs', 'through'],
 ['furchtbare', 'terrible'],
 ['kabin', 'cabin'],
 ['irgendein', 'any'],
 ['Mittagsgeschäft', 'Lunch business'],
 ['lenkt', 'directs'],
 ['Schinken', 'Ham'],
 ['aufgesetzt', 'put on'],
 ['Eintopf', 'stew'],
 ['Fingerabdrücke', 'fingerprints'],
 ['höheren', 'higher'],
 ['blass', 'pale'],
 ['versteckten', 'hidden'],
 ['staatenübergreifenden', 'transnational'],
 ['Mittelpunkt', 'Focus'],
 ['Häftling', 'prisoner'],
 ['vermisste', 'missed'],
 ['gekannt', 'known'],
 ['Verbrecher', 'criminal'],
 ['junges', 'young'],
 ['aufzufrischen', 'to refresh'],
 ['Kanne', 'pot'],
 ['Vermögen', 'Assets'],
 ['angeschossen', 'shot'],
 ['Baseballschläger', 'Baseball bat'],
 ['Totschläger', 'Blackjack'],
 ['erstellt', 'created'],
 ['holst', 'fetch'],
 ['schnappst', 'snap'],
 ['Hörzu', 'Listen'],
 ['Seiteneingang', 'Side entrance'],
 ['schlägst', 'beats'],
 ['sicherheitsbeamt', 'security officer'],
 ['spaziert', 'walks'],
 ['straßensorten', 'street varieties'],
 ['gestohlenen', 'stolen'],
 ['bereiten', 'prepare'],
 ['fing', 'caught'],
 ['jahrelang', 'for years'],
 ['Steinwolle', 'Rock wool'],
 ['Touristen', 'tourists'],
 ['Kennzeichen', 'Mark'],
 ['sicheres', 'safe'],
 ['merken', 'notice'],
 ['reingekommen', 'came in'],
 ['abgedeckt', 'covered'],
 ['Klimaanlage', 'air conditioner'],
 ['Hörzu', 'Listen'],
 ['schwitzt', 'sweats'],
 ['nass', 'wet'],
 ['Schweiß', 'Sweat'],
 ['riesige', 'huge'],
 ['klappen', 'succeed'],
 ['Weilchen', 'while'],
 ['fünftes', 'fifth'],
 ['Wasserhahn', 'water tap'],
 ['drehst', 'turn'],
 ['Gleichgewicht', 'balance'],
 ['festhalten', 'hold onto'],
 ['weiterhin', 'furthermore'],


 ];
